...
v5.1 — Institutional Tick Momentum Engine (fast path)

Same selectivity as v5, rebuilt for speed and dashboard calm:

- Rejections no longer rewrite last_signal_score/breakdown, no longer
  flap the pattern stage, and no longer log at INFO. The dashboard only
  sees state change on real events: trend birth/dissolution, pullback
  formation, or an actual signal.
- PRE_FETCH is no longer emitted on the tick stream. The engine's
  background prefetch loop already keeps a quote warm whenever the
  pattern stage is TREND/PULLBACK/MOMENTUM, so nothing is lost.
- All per-tick math is single-pass: tick deltas are computed once and
  shared by the noise, consistency, whipsaw, and efficiency checks.

Public interface is unchanged:
process_tick / get_state / on_trade_executed / on_signal_skipped /
update_mtf_bias / reset.
"""
...
# ---------------------------------------------------------------------------
# v5 quality profiles (selected by entry_score_threshold)
# ---------------------------------------------------------------------------
_PROFILES = {
    "aggressive": {
...
def _prior_window_velocity(ticks: List[float], window: int) -> Optional[float]:
    ...
    before the most recent `window` ticks."""
...
def _efficiency_ratio_from_deltas(
    net: float, deltas: List[float]
) -> float:
    """Kaufman's Efficiency Ratio from precomputed deltas."""
    path = 0.0
...
def _consistency_from_deltas(deltas: List[float], direction: str) -> float:
    """Fraction of non-flat deltas agreeing with `direction` (single pass)."""
    up = direction == "UP"
    with_trend = 0
    against = 0
...
def _whipsaw_ratio_from_deltas(deltas: List[float]) -> float:
    """Ratio of direction flips to non-flat moves (single pass).

    0.0 = clean one-directional push.
    1.0 = every tick reverses against the previous tick.
    """
    previous_sign = 0
    changes = 0
    moves = 0
...
def _micro_bias(ticks: List[float], window: int) -> Optional[str]:
    """Very-short-term direction from the live tick flow."""
...
def _htf_vote_counts(
    ...
    """Return (with_votes, against_votes, total_votes) from candle MTF."""
...
class StrategyEngine:
    """v5.1 institutional-quality tick momentum strategy (fast path)."""
    ...
    def __init__(
        ...
        # Only rewritten when a real signal fires — this is what keeps the
        # dashboard calm while low-quality setups are rejected silently.
        ...
        # v5 impulse tracking and rejection control.
    ...
    def process_tick(self, price: float) -> Optional[str]:
        """Process one tick. Returns 'BUY', 'SELL', or None."""
    ...
    def on_trade_executed(self) -> None:
        ...
        # After a real order, disable the same immediate early-capture path
        # for the remainder of this trend. Further entries must be fresh
    ...
    def on_signal_skipped(self) -> None:
        """Called when the engine saw a valid signal but did not execute it."""
        ...
        # A skipped signal was still a valid signal, not a low-quality reject.
        self._consecutive_rejects = 0
        self._last_reject_reason = None
    ...
    def _update_trend(self, ticks: List[float]) -> None:
        ...
        if direction != self._trend_direction:
            ...
            if self._trend_extreme_price is None:
                ...
                self._trend_extreme_price = max(
                    self._trend_extreme_price,
                    current_price,
                )
            else:
                self._trend_extreme_price = min(
                    self._trend_extreme_price,
                    current_price,
                )
    ...
    def _passes_regime_filters(ticks: List[float], window: int) -> bool:
        """Optional volatility/acceleration regime filters from config."""
    ...
    def _scan_windows(
        ...
        efficiency ratio clears `threshold`."""
    ...
    def _reject_signal(
        ...
        """Quietly park a low-quality setup.

        Deliberately does NOT touch pattern_stage, last_signal_score, or
        last_signal_score_breakdown: those feed the dashboard, and a
        rejection is not a new event worth re-rendering for. The pattern
        keeps tracking the market so a genuine setup forming right after
        is still caught.
        """
        base = (
        ...
        logger.debug(
            "Setup rejected: %s | quiet=%d ticks",
            reason,
    ...
    def _evaluate_entry(self, price: float, ticks: List[float]) -> Optional[str]:
        ...
        # Mode A: immediate early-capture entry.
        if (
    ...
    # ------------------------------------------------------------------
    # v5 institutional filters (single-pass where possible)
    # ------------------------------------------------------------------
    def _passes_entry_filters(
        ...
        # Compute deltas once; every quality check below reuses them.
        deltas = _deltas(sample)
        net = sample[-1] - sample[0]
        signed_net = net if direction == "UP" else -net

        # 1. Net displacement must agree with the signal.
        if signed_net <= 0:
        ...
        if noise is not None and noise > 1e-12:
            ...
            if displacement_ratio < limits["min_displacement_noise"]:
                return (
                    ...
                    f"below {limits['min_displacement_noise']:.2f}",
                )

        # 5. Whipsaw / chop filter.
        whipsaw = _whipsaw_ratio_from_deltas(deltas)

        if whipsaw > limits["max_whipsaw_ratio"]:
            return (
                False,
                f"choppy whipsaw {whipsaw:.2f} above "
                f"{limits['max_whipsaw_ratio']:.2f}",
            )

        # 6. Micro-bias protection.
        require_micro = (
        ...
        # 8. Immediate-entry protection: reject a strong counter-trend print.
        ...
        # 9. Pullback-specific quality filters.
        if entry_mode == "pullback":
            ...
            if retrace < limits["min_pullback_retrace"]:
                return (
                    False,
                    f"pullback too shallow {retrace:.2f} "
                    f"below {limits['min_pullback_retrace']:.2f}",
                )

            if retrace > limits["max_pullback_retrace"]:
                return (
                    False,
                    f"pullback too deep {retrace:.2f} "
                    f"above {limits['max_pullback_retrace']:.2f}",
                )
                ...
                if noise is not None and noise > 1e-12:
                    ...
                    if continuation_strength < limits["continuation_noise_mult"]:
                        return (
                            ...
                            f"below {limits['continuation_noise_mult']:.2f}",
                        )
...
class MTFAnalyzer:
    ...
    def analyze_with_strength(
        ...
        """Return direction, agreement count, and per-timeframe bias dict."""
        votes: List[str] = []
        tf_biases: Dict[str, str] = {}
