...
Trades only on a clear higher-timeframe trend, confirmed across several
independent lenses instead of a single EMA check:

- 30m and 1h must agree on direction AND be individually strong (ADX + DI),
  not just "EMA fast above EMA slow" (hard requirement).
- A closed 15m candle provides the trigger (no mid-candle repainting).
- The 15m entry candle itself must show real trend strength (its own ADX),
  a momentum-confirming MACD histogram, a healthy (non-exhausted) RSI zone,
  intact market structure (higher-lows for longs / lower-highs for shorts),
  and is boosted if it also prints a recognizable price-action pattern
  (engulfing / pin-bar rejection).
- A divergence guard hard-rejects setups where price is pushing further but
  RSI is not confirming it (classic exhaustion warning) — this is the kind
  of thing a "basic" EMA-cross bot walks straight into.
- 5m adds an alignment bonus.
- Adaptive ATR-normalised gates: volatility band, extension guard.

Signals are produced only when a new 15m candle closes, so the engine stays
calm between candles and reacts the instant that candle confirms — it does
not repaint and it does not wait longer than it has to. Public interface
...
def _candlestick_pattern_score(candle, prev, direction: str) -> int:
    """0/1/2 bonus for a recognizable price-action pattern aligned with `direction`."""
...
class _TimeframeTracker:
    """O(1)-per-candle indicator state for one timeframe: EMA/ATR trend engine
    plus RSI, MACD, ADX/DI, and lightweight market-structure tracking."""
    ...
    def _advance(self, candle) -> None:
        """Absorb one closed candle and roll every indicator forward by one step."""
        close = _safe_float(candle.get("close"))
        high = _safe_float(candle.get("high"))
        low = _safe_float(candle.get("low"))

        prior_candle = self.last_candle  # candle immediately before this one
        ...
        # --- RSI (Wilder-style smoothed gain/loss) ---
        if self.prev_close > 0:
        ...
        if prior_candle is not None and tr is not None:
            ...
            if self.adx is None:
                ...
                self.adx += (dx - self.adx) / ADX_PERIOD

        # --- lightweight market structure (swing highs/lows) ---
    ...
    def structure_score(self, direction: str) -> int:
        """0/1/2 bonus for intact market structure (higher-lows / lower-highs)."""
        if direction == "BUY":
    ...
    def divergence_against(self, direction: str) -> bool:
        """True if price is extending in `direction` while RSI is not confirming
        it (classic exhaustion warning that a plain EMA-cross bot would miss)."""
...
class StrategyEngine:
    """Multi-factor confluence strategy: trades only on a clear, strong 30m+1h
    trend with a 15m trigger that is itself confirmed across ADX, MACD, RSI
    zone, price-action pattern and market structure, with a divergence guard."""
    ...
    def _evaluate_entry_candle(self, tracker: _TimeframeTracker) -> None:
        ...
        # Gate 2: the entry timeframe itself must show a real trend (ADX), not
        # just ride the coattails of the 30m/1h bias while 15m chops sideways.
        if tracker.adx < ADX_MIN_TREND:
            self._reject(f"entry-tf ADX {tracker.adx:.1f} below {ADX_MIN_TREND} - 15m is choppy")
            return

        # Gate 3: adaptive volatility band.
        ...
        # Gate 5: RSI/price divergence guard — reject moves where price is
        # extending but momentum (RSI) is quietly failing to confirm it.
        if tracker.divergence_against(signal):
            self._reject("RSI/price divergence against trade direction - momentum fading")
            return

        # --- Scoring (max 25) ---
        trend_score = 5  # 30m + 1h agreement (ADX+DI confirmed) is mandatory to reach here
        ...
        if tracker.adx >= 35:
            ...
            adx_score = 1  # already passed the ADX_MIN_TREND gate above
        ...
        if macd_aligned and macd_rising:
            macd_score = 2
        elif macd_aligned:
            macd_score = 1
        else:
        ...
        score = (
            trend_score + trigger_score + momentum_score + volatility_score
