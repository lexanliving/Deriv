"""
src/strategy.py
MomentumMaster TF — v7 "Meridian" efficient candle-trend engine.

- Incremental EMA-20/50, slope, ATR (O(1) per newly closed candle).
- Skip-on-no-change: most 30s refreshes end after a cheap epoch scan.
- Gap-safe resync on irregular candle history.
- Bounded memory; signals expire after 120s.

Trade logic:
- 1h + 30m must agree on direction (hard requirement).
- 15m closed candle provides the trigger (no mid-candle repainting).
- 5m provides an alignment bonus.
...
class StrategyEngine:
    def __init__(
        ...
        **_: Any,
        ...
        logger.info(
            "Meridian v7 initialised | entry_threshold=%d/%d | entry_tf=%s | trend_tfs=%s",
