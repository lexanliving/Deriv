...
Trades only on a clear higher-timeframe trend (30m + 1h agreement, ADX+DI
confirmed) with a closed 15m trigger, itself confirmed across ADX, MACD, RSI
zone, price-action pattern and market structure, plus a divergence guard and
adaptive ATR-normalised gates. Signals fire only on a new 15m candle close.
Also exposes a full per-candle evaluation record for the decision journal.
"""
import time
import uuid
