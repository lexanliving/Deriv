...
Higher-timeframe trend confirmation remains:

    30m + 1h

The journal behaviour is unchanged. The strategy still produces one evaluation
per new trigger candle close.
"""
import time
import uuid
...
class _TimeframeTracker:
    ...
    def absorb(self, closed) -> bool:
        ...
        newest_epoch = int(_safe_float(closed[-1].get("epoch"), 0.0))
        if newest_epoch <= 0:
            return False
        ...
        if newest_epoch <= self.last_epoch:
            return False
        ...
        fresh = [
            c
            for c in closed
            if int(_safe_float(c.get("epoch"), 0.0)) > self.last_epoch
        ]
    ...
    def _advance(self, candle) -> None:
        ...
        if self.ema12 is None:
            ...
            if self._macd_signal is None:
                ...
                self._macd_signal += (2.0 / (MACD_SIGNAL + 1)) * (
                    macd_line - self._macd_signal
                )
        ...
        if prior_candle is not None and tr is not None:
            ...
            dx = (
                100.0 * abs(self.di_plus - self.di_minus) / di_sum
                if di_sum > 1e-12
                else 0.0
            )
        ...
        self.close = close
        self.last_epoch = int(_safe_float(candle.get("epoch"), 0.0))
...
class StrategyEngine:
    ...
    def __init__(
        ...
        **_: Any,
        ...
        self._entry_tf = str(
            entry_timeframe
            or ENTRY_TIMEFRAME_BY_DURATION.get(
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "entry_adx": round(entry_tracker.adx, 1)
            if entry_tracker and entry_tracker.adx is not None
            else None,
            "entry_rsi": round(entry_tracker.rsi, 1)
            if entry_tracker and entry_tracker.rsi is not None
            else None,
            "entry_macd_hist": round(entry_tracker.macd_hist, 5)
            if entry_tracker
            else None,
        }
    ...
    def _update_trend_bias(self) -> None:
        self._mtf_tf_biases = {
            tf: t.bias
            for tf, t in self._trackers.items()
        }
    ...
    def _evaluate_entry_candle(self, tracker: _TimeframeTracker) -> None:
        ...
        if tracker.adx < self._entry_adx_floor:
            self._reject(
                f"entry-tf ADX {tracker.adx:.1f} below {self._entry_adx_floor} - "
                f"{self._entry_tf} is choppy"
            )
            return
        ...
        if atr_ratio < vol_floor:
            self._reject(
                f"market too quiet right now for a {self._contract_minutes}m contract"
            )
            return
        ...
        if self._regime == "SHORT":
            ...
            if five.adx is None or five.adx < REGIME_SHORT_5M_ADX_FLOOR:
                self._reject(
                    "5m not actually trending, only directionally biased "
            ...
            if h.adx is None or h.adx < REGIME_LONG_1H_ADX_FLOOR:
                self._reject(
                    f"1h ADX below {REGIME_LONG_1H_ADX_FLOOR} - trend too weak "
                    "to hold for 60m+"
                )
                return
            ...
            if not h_macd_aligned:
                self._reject(
                    "1h MACD not aligned with the trade (required for contracts >=60m)"
                )
                return
        ...
        momentum_score = (
            ...
            if directional_close_position >= 0.60
            else 1
            if directional_close_position >= 0.50
            else 0
        )
        ...
        alignment_score = (
            1
            if self._mtf_tf_biases.get("5m") == self._trend_direction
            else 0
        )
        ...
        macd_aligned = (
            tracker.macd_hist > 0
            if signal == "BUY"
            else tracker.macd_hist < 0
        )

        macd_trail = list(tracker.macd_hist_trail)
        macd_rising = len(macd_trail) >= 2 and (
            macd_trail[-1] > macd_trail[-2]
            if signal == "BUY"
            else macd_trail[-1] < macd_trail[-2]
        )
        ...
        if score >= self._entry_score_threshold:
            ...
            logger.info(
                "CONFLUENCE %s | %d/%d | regime=%s | entry_tf=%s | %s | close=%.5f atr=%.5f adx=%.1f rsi=%.1f",
                signal,
                score,
                SCORE_MAX,
                self._regime,
