...
def _candle_slope_bias(closes: List[float]) -> Optional[str]:
    ...
    Instead of just comparing close[-1] vs close[-3] (which can be flipped
    by a single outlier candle), this fits a simple linear regression slope
    across all closes and uses its sign as the bias direction.

    Returns 'UP', 'DOWN', or None if the slope is too flat to be meaningful.
    """
    n = len(closes)
    if n < 3:
        return None
    ...
    # Require the slope to be at least 0.5% of the mean price to avoid
    # calling a flat candle series as directional.
    if y_mean == 0:
        return None
    relative_slope = abs(slope) / abs(y_mean)
    if relative_slope < 0.0005:
        return None
...
class StrategyEngine:
    ...
    def _signal_if_confirmed(self, trend: str) -> Optional[str]:
        ...
        # ------------------------------------------------------------------
        cont_pts = 0
        # Need at least: [pre-pullback tick, pullback tick, cont tick 1, cont tick 2]
        if len(ticks) >= 4:
            pullback_size = abs(ticks[-3] - ticks[-4])
            cont_size = abs(ticks[-1] - ticks[-3])
            if pullback_size > 0 and cont_size >= pullback_size * 1.5:
...
class MTFAnalyzer:
    ...
    def _analyze_single_tf(candles: List[Dict], label: str) -> Optional[str]:
        ...
        v3: Uses linear regression slope across all candle closes instead of
        a simple close[-1] vs close[-3] comparison. This is more robust
        against single-candle outliers.
        """
        if len(candles) < 3:
        ...
        try:
            ...
            return None
        return _candle_slope_bias(closes)
