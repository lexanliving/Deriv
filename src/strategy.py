...
Trades only on a strong 30m+1h trend with a closed 15m trigger confirmed across
ADX, MACD, RSI zone, price-action pattern and market structure, with a divergence
guard and adaptive ATR gates. Signals fire only on a new 15m candle close.

Duration-aware regimes (SHORT/MEDIUM/LONG) change WHICH conditions qualify a
setup, while the 25-point score and its weights stay fixed so the threshold
slider keeps one meaning. A universal headwind rule (all regimes) never enters
while the 5m is actively working against the trade.

Each per-candle evaluation record now also carries `regime` and `duration_min`
so the Performance Scope can analyse win rate by contract length — purely
additive metadata, no change to any gate or score.
"""
import time
import uuid
