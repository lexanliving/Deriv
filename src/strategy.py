...
setup; the 25-point score and its weights stay fixed so the threshold slider
keeps one meaning. The volatility gate is a DIRECT current-candle movement test
(REGIME_VOL_BAND) — not a percentile of own history.

Each evaluation record also carries the per-timeframe bias snapshot
(tf_5m/15m/30m/1h + mtf_agreement), regime and duration, so the journal/Scope can
show the full higher-timeframe context of every decision. Each StrategyEngine
instance holds its own trackers — no shared/class-level state — so multiple tabs
analyse their markets fully independently.
"""
import time
import uuid
...
from config import (
    ...
    REGIME_SHORT_5M_ADX_FLOOR, REGIME_TRIGGER_BODY_MIN, REGIME_VOL_BAND,
...
class StrategyEngine:
    ...
    def _evaluate_entry_candle(self, tracker: _TimeframeTracker) -> None:
        ...
        if tracker.adx < self._entry_adx_floor:
            ...
            return
        # Volatility gate — DIRECT current-candle movement, per-regime band.
