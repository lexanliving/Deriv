...
causes:
  ...
  1. The candle-based MTF bias (5m/15m/30m regression) lags a fresh
  ...
  2. MTF only refreshed every 60s (15s in-setup), so the vote flipped
     long after the move had finished.
  3. The scoring gate demanded momentum *acceleration* (>=1.2x the prior
  ...
  4. The entry pattern required an exact pullback -> 2-continuation-tick
...
class StrategyEngine:
    ...
    def _evaluate_entry(self, price: float, ticks: List[float]) -> Optional[str]:
        ...
        if self._trend_age <= self._early_trend_max_age and self._pattern_stage in ("TREND", "IDLE"):
    ...
    def _update_pattern(
        self, price: float, ticks: List[float], trend: str, signal: str
    ) -> Optional[str]:
    ...
    def _signal_if_confirmed(self, trend: str, signal: str, ticks: List[float]) -> Optional[str]:
        ...
        logger.info(
            ...
            signal, score, self._entry_score_threshold, breakdown,
    ...
    # ------------------------------------------------------------------
    def _score_signal(
        self, signal: str, ticks: List[float], entry_mode: str
        ...
        candle_votes = [v for k, v in self._mtf_tf_biases.items()
