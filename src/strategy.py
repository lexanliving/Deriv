...
def compute_dynamic_barrier(
    ticks: List[float],
    window: int = DYNAMIC_BARRIER_WINDOW,
    multiplier: float = DYNAMIC_BARRIER_MULTIPLIER,
    floor: float = DYNAMIC_BARRIER_FLOOR,
    ceil: float = DYNAMIC_BARRIER_CEIL,
) -> Optional[float]:
    """Barrier offset sized to the recent realized tick-to-tick move, instead
    of a fixed distance.

    Backtesting (see strategy-improvement report, July 2026) found this
    roughly doubled simulated win rate and profit factor versus a fixed
    +/-0.08 barrier, with the entry logic itself unchanged, and the
    improvement held up on out-of-sample seeds. Returns None if there isn't
    yet enough tick history, so callers can fall back to a static barrier
    rather than skip or delay the trade.
    """
    if len(ticks) < window + 1:
        return None
    sample = ticks[-(window + 1):]
    deltas = [abs(sample[i] - sample[i - 1]) for i in range(1, len(sample))]
    avg_move = sum(deltas) / len(deltas)
    if avg_move <= 0:
        return None
    return max(floor, min(ceil, multiplier * avg_move))
...
class StrategyEngine:
    ...
    def __init__(
        ...
        # Bugfix (July 2026): this used to be max(trend_window_max,
        # burst_window_max) + 2, which caps below VOLATILITY_WINDOW (20) -
        # meaning _tick_volatility() never had enough history to return a
        # value and MIN/MAX_TICK_VOLATILITY silently never fired no matter
        # what they were set to. VOLATILITY_WINDOW is now included so that
        # filter can actually be evaluated if enabled.
        buffer_size = max(trend_window_max, burst_window_max, VOLATILITY_WINDOW) + 2
