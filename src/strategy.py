...
candle close (no repainting). Also exposes a full per-candle evaluation record
(taken or rejected) for the decision log.
"""
import time
import uuid
...
class StrategyEngine:
    def __init__(
        ...
        **_: Any,
        ...
        logger.info(
            "Confluence engine ready | threshold=%d/%d | entry=%s | confirm=%s",
    ...
    def _evaluate_entry_candle(self, tracker: _TimeframeTracker) -> None:
        ...
        if score >= self._entry_score_threshold:
            ...
            logger.info(
                "CONFLUENCE %s | %d/%d | %s | close=%.5f atr=%.5f adx=%.1f rsi=%.1f",
