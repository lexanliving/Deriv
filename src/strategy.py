...
v5 upgrade — Institutional Tick Momentum Engine

Goal:
- Keep the best early-trend capture behaviour from v4.
- Remove low-quality dummy entries in choppy, ragged, or exhausted markets.
- Make every sensitivity profile effective:
    Aggressive  = more opportunities, but still quality-filtered.
    Balanced    = strong all-round filter.
    Conservative = only the cleanest high-conviction trades.

The strategy keeps the exact public interface used by TradingEngine:
...
# v5 quality profiles
# ---------------------------------------------------------------------------
# The active profile is inferred from entry_score_threshold so existing
# config presets continue to work without modification:
#
#   threshold <= 5  -> aggressive
#   threshold <= 8  -> balanced
#   threshold >= 9  -> conservative
#
# Each profile relaxes or tightens the same institutional filters.
_PROFILES = {
    "aggressive": {
...
def _tick_consistency(sample: List[float], direction: str) -> float:
    """Fraction of non-flat tick moves that agree with `direction`."""
    if len(sample) < 2:
        return 0.0
...
def _delta_noise(sample: List[float]) -> Optional[float]:
    """Population stdev of tick deltas inside `sample`."""
    if len(sample) < 3:
        return None
...
def _signed_net_displacement(sample: List[float], direction: str) -> float:
    """Return positive net displacement when sample agrees with direction."""
    if len(sample) < 2:
        return 0.0
...
def _displacement_noise_ratio(sample: List[float], direction: str) -> float:
    """Net directional displacement divided by recent tick noise."""
    net = _signed_net_displacement(sample, direction)

    if net <= 0:
        return -1.0

    noise = _delta_noise(sample)

    if noise is None or noise <= 1e-12:
        return 100.0

    return net / noise


def _whipsaw_ratio(sample: List[float]) -> float:
    """Ratio of direction flips to non-flat tick moves.
...
class StrategyEngine:
    """v5 institutional-quality tick momentum strategy."""
    ...
    # Entry evaluation
    # ------------------------------------------------------------------
    def _maybe_prefetch(self) -> Optional[str]:
    ...
    def _reject_signal(
        ...
        logger.info(
            "Signal rejected: %s | quiet=%d ticks",
            reason,
    ...
    def _passes_entry_filters(
        ...
        # 3. Tick consistency.
        consistency = _tick_consistency(sample, direction)
        ...
        displacement_ratio = _displacement_noise_ratio(sample, direction)
        ...
        # 8. Immediate-entry specific protection:
        #    reject if the current tick is a strong counter-trend print.
