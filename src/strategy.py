...
from config import (
    ...
    MACD_FAST, MACD_SIGNAL, MACD_SLOW, MTF_MIN_AGREEMENT,
    REGIME_EXHAUSTION_ATR, REGIME_LONG_1H_ADX_FLOOR, REGIME_SHORT_5M_ADX_FLOOR,
    REGIME_TRIGGER_BODY_MIN, REGIME_VOL_BAND, RSI_PERIOD, TREND_TIMEFRAMES,
)
...
def _candle_metrics(candle) -> Dict[str, float]:
    ...
    return {
        ...
        "range": candle_range, "body_ratio": body / candle_range,
...
class StrategyEngine:
    ...
    def __init__(self, entry_score_threshold: int = ENTRY_SCORE_THRESHOLD,
                 contract_duration_minutes: int = 30, entry_adx_floor: int = ADX_MIN_TREND, **_: Any):
        ...
        logger.info(
            "Confluence engine ready | threshold=%d/%d | %s ADX floor=%d | regime=%s (%dm) | entry=%s confirm=%s",
            self._entry_score_threshold, SCORE_MAX, self._entry_tf, self._entry_adx_floor,
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            "trend_direction": self._trend_direction, "trend_tick_count": 0,
            "trend_kind": "candle", "trend_age": 0,
            "trades_in_trend": self._trades_in_trend, "in_cooldown": self._in_cooldown,
            "pattern_stage": self._pattern_stage, "mtf_bias": self._mtf_bias,
            "mtf_agreement": self._mtf_agreement, "mtf_tf_biases": dict(self._mtf_tf_biases),
            "micro_bias": None, "last_signal_score": self._last_signal_score,
    ...
    def _build_evaluation(self, tracker) -> Dict[str, Any]:
        ...
        if self._pending_signal:
            ...
            direction = "-"
        return {
            "signal_id": self._pending_signal_id or "", "direction": direction,
            "trend": self._trend_direction or "-",
            "taken": "TRUE" if self._pending_signal else "FALSE",
            "rejection_reason": self._last_rejection or "",
            "score": self._last_signal_score, "threshold": self._entry_score_threshold,
            "regime": self._regime, "duration_min": self._contract_minutes,
            "s_trend": bd.get("trend", ""), "s_trigger": bd.get("trigger", ""),
            "s_momentum": bd.get("momentum", ""), "s_volatility": bd.get("volatility", ""),
            "s_alignment": bd.get("alignment", ""), "s_adx": bd.get("adx", ""),
            "s_macd": bd.get("macd", ""), "s_rsi_zone": bd.get("rsi_zone", ""),
            "s_pattern": bd.get("pattern", ""), "s_structure": bd.get("structure", ""),
    ...
    def _sync_state_version(self) -> None:
        signature = (
            self._trend_direction, self._pattern_stage, self._mtf_bias, self._mtf_agreement,
            self._last_entry_mode, self._last_signal_score, self._pending_signal,
            self._mtf_sig, self._breakdown_sig,
        )
