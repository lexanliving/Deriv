"""
src/strategy.py — MomentumMaster TF confluence engine ("Meridian").
Trades only on a strong 30m+1h trend with a closed 15m trigger that is itself
confirmed across ADX, MACD, RSI zone, price-action pattern and market structure,
with a divergence guard and adaptive ATR gates. Signals fire only on a new 15m
candle close (no repainting).

Duration-aware: the contract length selects a regime that changes WHICH
conditions qualify a setup (the analysis differs by timeframe), while the
25-point score and its weights stay fixed so the threshold slider keeps its
meaning:
  SHORT  (<=15m) : 30m leads, 1h must not oppose, 5m must be TRENDING (not
                   just biased) in the trade direction, and the 15m trigger
                   candle must be genuinely decisive. Wider exhaustion room
                   and a higher volatility floor — a fast contract needs
                   real movement happening right now.
  MEDIUM (30m)   : 30m and 1h must agree (the original, default behaviour).
  LONG   (>=60m) : 30m and 1h must agree, 1h structure must be intact, 1h
                   ADX must show a real trend, and 1h MACD must agree too.
                   Tighter exhaustion room — a full hour gives mean-reversion
                   more time to bite, so entries must sit closer to the EMA.
These are gates (filters only); they never rescale the score.

Also exposes a full per-candle evaluation record (taken or rejected) for the
decision log.
"""
import time
import uuid
...
from config import (
    ...
    MTF_MIN_AGREEMENT, REGIME_EXHAUSTION_ATR, REGIME_LONG_1H_ADX_FLOOR,
    REGIME_SHORT_5M_ADX_FLOOR, REGIME_TRIGGER_BODY_MIN, REGIME_VOL_PERCENTILE,
    RSI_PERIOD, TREND_TIMEFRAMES,
)
...
class _TimeframeTracker:
    ...
    def _refresh_bias(self) -> None:
        # Stable definition of "trending" (module ADX_MIN_TREND); the preset's
        # entry_adx_floor only tightens the 15m ENTRY gate, not these biases.
...
class StrategyEngine:
    @staticmethod
    def _regime_for(minutes: int) -> str:
        if minutes <= 15:
            return "SHORT"
        if minutes <= 30:
            return "MEDIUM"
    ...
    def __init__(
        ...
        contract_duration_minutes: int = 30,
        ...
        **_: Any,
        ...
        logger.info(
            "Confluence engine ready | threshold=%d/%d | 15m ADX floor=%d | regime=%s (%dm) | entry=%s confirm=%s",
            self._entry_score_threshold, SCORE_MAX, self._entry_adx_floor,
            self._regime, self._contract_minutes, self._entry_tf, self._trend_tfs,
        )
    ...
    def consume_signal(self) -> Optional[str]:
        ...
        if time.time() - self._pending_signal_time > SIGNAL_MAX_AGE_SECONDS:
            logger.info("Dropped stale %s signal.", signal)
    ...
    def _update_trend_bias(self) -> None:
        # Regime-aware trend decision: the analysis differs by contract length.
        ...
        if self._regime == "SHORT":
            # 30m leads; 1h only needs to NOT oppose; 5m is enforced as a gate later.
        ...
        else:  # MEDIUM / LONG: 30m and 1h must agree.
    ...
    def _evaluate_entry_candle(self, tracker: _TimeframeTracker) -> None:
        ...
        if separation < 0.30 * atr:
            ...
            return
        # 15m ADX gate uses the preset floor (real selectivity lever), not the
        # stable bias floor.
        if tracker.adx < self._entry_adx_floor:
            self._reject(f"entry-tf ADX {tracker.adx:.1f} below {self._entry_adx_floor} - 15m is choppy")
            return
        if len(self._atr_history) >= 24:
            vol_floor, vol_ceiling = REGIME_VOL_PERCENTILE.get(self._regime, (0.10, 0.95))
            vol_rank = _percentile_rank(self._atr_history, atr)
            if vol_rank < vol_floor:
                self._reject(f"volatility too dead for a {self._contract_minutes}m contract")
                return
            if vol_rank > vol_ceiling:
        ...
        # --- Duration-specific gates (the analysis that differs by length) ---
        if self._regime == "SHORT":
            # A 5-15m contract expires fast, so the trigger candle itself must
            # be genuinely decisive (not just barely qualifying on score).
            if body_ratio < REGIME_TRIGGER_BODY_MIN:
                self._reject(f"trigger candle too weak (body {body_ratio:.2f} < "
                             f"{REGIME_TRIGGER_BODY_MIN}) for a contract <=15m")
                return
            ...
            if five.adx is None or five.adx < REGIME_SHORT_5M_ADX_FLOOR:
                self._reject("5m not actually trending, only directionally biased "
                             "(required for contracts <=15m)")
                return
        elif self._regime == "LONG":
            # A 60m+ hold needs a trend that can plausibly last the full
            # duration — intact structure alone isn't proof of that.
            ...
            if h.adx is None or h.adx < REGIME_LONG_1H_ADX_FLOOR:
                self._reject(f"1h ADX below {REGIME_LONG_1H_ADX_FLOOR} - trend too weak "
                             "to hold for 60m+")
                return
        ...
        # --- Scoring (max 25) — weights fixed so the threshold keeps meaning ---
        trend_score = 5
        ...
        if score >= self._entry_score_threshold:
            ...
            logger.info(
                "CONFLUENCE %s | %d/%d | regime=%s | %s | close=%.5f atr=%.5f adx=%.1f rsi=%.1f",
                signal, score, SCORE_MAX, self._regime, breakdown, close, atr, tracker.adx, rsi,
            )
        else:
