...
def _mm_patch_width():
    """Rewrite the deprecated use_container_width kwarg to width='stretch'.
    We patch unconditionally (Streamlit hides `width` behind **kwargs, so a
    signature check would wrongly skip it) and fall back to the old kwarg if a
    build truly lacks `width` — so warnings vanish with zero layout change."""
...
def _arrow_safe(rows):
    """Make a list-of-dicts Arrow-serializable for st.dataframe: stringify any
    dict/list cell, and stringify whole columns that mix str with int/float
    (the exact mix that made pyarrow throw on `weights` / `threshold`)."""
    if not rows:
        return rows
...
with tab_p:
    st.markdown('<div class="tb-panel"><div class="tb-h">Provider chain <span class="tag">Groq → OpenRouter → Cerebras → OpenAI</span></div>'
                '<div class="tb-note">Requests try each configured provider in this order and roll over on rate-limits, stale models, host-blocks, or outages. '
                'A stale default model is auto-replaced by a live one the provider advertises. '
                'If Groq returns 403/1010 it is blocking the host IP/key — that is not fixable from code; add OpenRouter (free) as a backstop so the chain has somewhere to roll.</div>', unsafe_allow_html=True)
    for n in nodes:
    ...
    if st.button("Write today’s synthesis", type="primary", disabled=(not nodes_configured or s["reviews"] == 0)):
        ...
        if diary:
            ...
            st.markdown(_trace_html(trace), unsafe_allow_html=True)
            st.markdown('<div class="tb-warn">No provider returned text. The trace above now states exactly why each one failed. '
                        'A Groq 403/1010 means Groq blocked this host — add OpenRouter or Cerebras (free) so the chain has a backstop on different infra. '
                        'A 401 means re-check the key; a 404 means set the *_MODEL env var to a current model.</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
