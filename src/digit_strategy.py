...
* Over 6 is considered only when 7/8/9 dominate 0-6 in both fast and
  medium windows, with slow-window support;
...
The quote-aware expected-value check is performed by the trading engine after
it requests the live proposal, because the ask price and payout are dynamic.
"""
...
class DigitStrategyEngine:
    ...
    def _window_stats(self, window: int) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "p_over6": round(over_count / window, 6),
            "p_low": round(low_count / window, 6),
            "p_1to6": round(comparison_count / window, 6),
            "dominance": round((over_count - comparison_count) / window, 6),
        }
    ...
    def _candidate(self, stats: Dict[str, Optional[Dict[str, Any]]]) -> Tuple[bool, str]:
        ...
        if fast["p_over6"] <= fast["p_1to6"]:
            return False, "7–9 do not exceed 1–6 in the fast window"
        if medium["p_over6"] <= medium["p_1to6"]:
            return False, "7–9 do not exceed 1–6 in the medium window"
        if slow["p_over6"] < 0.30 or slow["p_over6"] <= slow["p_1to6"]:
            return False, "slow window does not support 7–9 over 1–6"
        return True, "7–9 dominates 1–6 across fast and medium windows"
    ...
    def review_if_due(self, now: Optional[float] = None) -> Optional[Dict[str, Any]]:
        ...
        self._last_signal_score_breakdown = {
            ...
            "slow_over6_pct": round((stats.get("slow") or {}).get("p_over6", 0.0) * 100, 2),
            "rule": reason,
        }
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mtf_tf_biases": {name: ("UP" if (stats.get(name) or {}).get("p_over6", 0) > (stats.get(name) or {}).get("p_1to6", 1) else "FLAT") for name in self.windows},
