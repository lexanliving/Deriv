"""src/research_engine.py — sends each completed trade's research to Supabase and
logs every success/failure so you can confirm rows are flowing.
"""
...
class ResearchEngine:
    ...
    def _run(self):
        sb = get_supabase()
        schema_ok = sb.ensure_schema() if sb.enabled else False

        logger.info("Research pipeline | supabase_enabled=%s schema_ok=%s", sb.enabled, schema_ok)
    ...
    def _process(self, tid):
        ...
        if sb.enabled:
            ...
            if ok_r and ok_k:
                logger.info("Supabase: research UPLOADED for trade %s (%s %s).", tid, trade.symbol, trade.status)
                self._mark_done(tid)
            else:
                logger.error("Supabase: upload FAILED for %s (%s). Queued to outbox.", tid, sb.last_error)
                self._append(OUTBOX_FILE, {"research": record, "knowledge": krows})
        else:
            logger.warning("Supabase disabled; research stored locally only (trade %s).", tid)
    ...
    def _flush_outbox(self):
        ...
        if not sb.enabled or not sb.ensure_schema() or not os.path.exists(OUTBOX_FILE):
            return
        ...
        for e in entries:
            ...
            if ok_r and ok_k:
                logger.info("Supabase: outbox row uploaded (trade %s).", e["research"].get("trade_id"))
        ...
        if entries:
            logger.info(
                "Outbox flush: %d uploaded, %d remaining.",
