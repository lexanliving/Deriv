...
class ResearchEngine:
        ...
        while not self._stop.is_set():
            ...
            if now - last_poll > 3.0:
                ...
                try:
                    ...
                    logger.error("Research poll failed: %s", exc)
            try:
            ...
            if tid:
                try:
                    ...
                    logger.error("Research processing failed for %s: %s", tid, exc)
                    self._mark_done(tid)
            if now - last_flush > 60.0:
                ...
                try:
                    ...
                    logger.error("Outbox flush failed: %s", exc)
    ...
    def _process(self, tid):
        ...
        symbol = getattr(trade, "symbol", "") or (jrow.get("symbol") if jrow else "") or ""
        ctx = {"symbol": symbol, "direction": trade.direction, "outcome": trade.status, "pnl": trade.pnl,
        ...
        record.update({"trade_id": tid, "signal_id": getattr(trade, "signal_id", ""), "symbol": symbol,
                       "direction": trade.direction, "outcome": trade.status, "pnl": trade.pnl, "model": model})
        krows = analytics.knowledge_rows_from(record)
        if sb.enabled:
            sb.link_trade(tid, {"symbol": symbol, "direction": trade.direction, "stake": trade.stake,
                                "opened_at": trade.timestamp, "closed_at": time.strftime("%Y-%m-%d %H:%M:%S"),
            ...
            if ok_r and ok_k:
                ...
                # The outbox owns the retry from here — do NOT leave the id
                # unmarked, or it would be re-processed (and duplicated) on
                # every poll.
                self._append(OUTBOX_FILE, {"research": record, "knowledge": krows})
                self._mark_done(tid)
                logger.error("Research upload failed for %s; queued in outbox.", tid)
        else:
