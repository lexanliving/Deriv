...
Polls finalized trades, queues them, generates AI research, uploads to Supabase
with retry, and persists failures to an outbox flushed every 60s. With no
Supabase secrets it still writes research locally (research_local.jsonl).
Also starts the Venture Engine (the AI 'discusser' gate).
"""
...
class ResearchEngine:
    def __init__(self) -> None:
        ...
        self._state = None
        self._done: Set[str] = set(self._load_done())
    ...
    def attach(self, state) -> None:
        ...
        try:
            from src.venture_engine import get_venture_engine
            get_venture_engine().attach(state)
    ...
    def _mark_done(self, tid: str) -> None:
        self._done.add(tid)
        self._append(DONE_FILE, {"trade_id": tid, "ts": time.time()})
    ...
    def _journal_row_for(self, signal_id: str) -> Optional[Dict[str, str]]:
        if not signal_id:
            return None
    ...
    def _process(self, tid: str) -> None:
        st = self._state
        trade = st.get_trade(tid) if st else None
        if trade is None:
            return
        jrow = self._journal_row_for(getattr(trade, "signal_id", ""))
        ctx = {
            "symbol": trade.symbol, "direction": trade.direction, "outcome": trade.status,
            "pnl": trade.pnl, "score": None, "threshold": None,
            "regime": jrow.get("regime") if jrow else None,
            "tf_biases": ({k: jrow.get(k) for k in ("tf_5m", "tf_15m", "tf_30m", "tf_1h")} if jrow else {}),
            "mae": analytics._f(jrow, "mae") if jrow else None,
            "mfe": analytics._f(jrow, "mfe") if jrow else None,
            "martingale_step": trade.martingale_step,
            "rejection": jrow.get("rejection_reason") if jrow else None,
            "factor_profile": analytics.factor_profile(jrow),
            "stats": analytics.aggregate_stats(get_journal().read_archive_merged()),
        }
        if jrow:
            ctx["score"] = analytics._f(jrow, "score")
            ctx["threshold"] = analytics._f(jrow, "threshold")
        ...
        record.update({"trade_id": tid, "signal_id": getattr(trade, "signal_id", ""),
                       "symbol": trade.symbol, "direction": trade.direction,
                       "outcome": trade.status, "pnl": trade.pnl, "model": model})
        krows = analytics.knowledge_rows_from(record)

        if sb.enabled:
            sb.link_trade(tid, {"symbol": trade.symbol, "direction": trade.direction,
                                "stake": trade.stake, "opened_at": trade.timestamp,
                                "closed_at": time.strftime("%Y-%m-%d %H:%M:%S"), "outcome": trade.status})
            ok_r = sb.upsert_research(record)
            ok_k = sb.upsert_knowledge(krows)
            if ok_r and ok_k:
                self._mark_done(tid)
            else:
                self._append(OUTBOX_FILE, {"research": record, "knowledge": krows})
                logger.error("Research upload failed for %s; queued to outbox.", tid)
        else:
            self._append(LOCAL_FILE, record)
            self._mark_done(tid)
    ...
    def _flush_outbox(self) -> None:
        ...
        remaining = []
        for e in entries:
            ok_r = sb.upsert_research(e.get("research", {}))
            ok_k = sb.upsert_knowledge(e.get("knowledge", []))
            if ok_r and ok_k:
        ...
        try:
            ...
            pass
        if entries:
            logger.info("Outbox flush: %d uploaded, %d remaining.",
                        len(entries) - len(remaining), len(remaining))


_singleton: Optional[ResearchEngine] = None
