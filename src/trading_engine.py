...
class TradingEngine:
    ...
    def __init__(
        ...
        self._strategy = StrategyEngine(
            ...
            momentum_confirm_ticks=STRICT_CONTINUATION_TICKS,
        )
        self._mtf_analyzer = MTFAnalyzer(min_agreement=preset["mtf_min_agreement"])
        self._last_mtf_refresh: float = 0.0
        self._take_profit_enabled = bool(take_profit_enabled)
        self._take_profit_amount = float(take_profit_amount)
        self._tp_task: Optional[asyncio.Task] = None
    ...
    async def _shutdown(self):
        ...
        if self._tp_task and not self._tp_task.done():
            self._tp_task.cancel()
            await asyncio.gather(self._tp_task, return_exceptions=True)
        self._tp_task = None
    ...
    async def _on_tick(self, tick_data: Dict[str, Any]):
        ...
        try:
            ...
            # Calculate dynamic EV if a signal is generated
            if signal in ["BUY", "SELL"]:
                ev_ok = self._validate_ev(signal, price)
                if not ev_ok:
                    logger.info(f"Signal {signal} rejected by EV engine.")
                    signal = None
            ...
            self.state.update_strategy_state(
                ...
                # NOTE: mtf_bias is NOT overwritten here — it is managed by _refresh_mtf
                # which uses get_display_bias() to always show the majority direction
                # even when below the strict min_agreement threshold.
            )
    ...
    def _validate_ev(self, signal: str, current_price: float) -> bool:
        """
        Calculate Expected Value (EV) for the proposed contract.
        EV = (P(touch) * Payout) - ((1 - P(touch)) * Stake)
        """
        try:
            # Estimate volatility from tick buffer
            ticks = list(self._strategy._tick_buffer)
            if len(ticks) < 10: return True # Not enough data, fail safe
            
            returns = [abs(ticks[i] - ticks[i-1]) for i in range(1, len(ticks))]
            sigma = sum(returns) / len(returns) # Mean absolute deviation as proxy for vol
            
            if sigma == 0: return True
            
            barrier_offset = float(self.barrier_buy if signal == "BUY" else self.barrier_sell)
            barrier_dist = abs(barrier_offset)
            duration = float(CONTRACT_DURATION)
            
            # Simple approximation of touch probability: 
            # P(touch) ~ 2 * P(price > barrier at expiry)
            # z = barrier / (sigma * sqrt(T))
            z = barrier_dist / (sigma * math.sqrt(duration))
            ...
            p_touch = 2 * (1 - norm_cdf(z))
            p_touch = max(0.01, min(0.99, p_touch))
            
            # Payout estimation (Touch contracts usually pay 1.8x to 2.5x)
            # We use a conservative 2.0x if we don't have a proposal yet
            estimated_payout = 2.0 
            stake = self.state.current_stake
            
            ev = (p_touch * (stake * estimated_payout)) - ((1 - p_touch) * stake)
            
            logger.info(f"EV Analysis: P(touch)={p_touch:.2f}, Sigma={sigma:.4f}, EV={ev:.2f}")
    ...
    async def _refresh_mtf(self):
        ...
        try:
            ...
            # For the dashboard, always show the majority direction even if below min_agreement
            display_bias, display_agreement = self._mtf_analyzer.get_display_bias(candles_by_tf)
            self.state.update_strategy_state(mtf_bias=display_bias, mtf_agreement=display_agreement)

            self._last_mtf_refresh = time.time()

            bias_str = f"{display_bias} ({display_agreement}/3)" if display_bias != "—" else "No data"
            trade_str = f"{bias} ({agreement}/3)" if bias else "blocked"
            logger.info(f"MTF display: {bias_str} | Trade gate: {trade_str}")
            self.state.set_status(f"Bot active | MTF: {bias_str} | Trade gate: {trade_str}")
    ...
    async def _execute_trade(self, signal: str, entry_price: float):
        ...
        try:
            ...
            # Step 2.5: Start Take Profit monitoring if enabled
            if self._take_profit_enabled and self._take_profit_amount > 0:
                self._tp_task = asyncio.create_task(
                    self._take_profit_loop(contract_id, buy_price),
                    name="tp-monitor",
                )
                logger.info(f"Take Profit monitoring active for contract {contract_id} (target: +{self._take_profit_amount:.2f})")
    ...
    async def _pre_fetch_loop(self):
        ...
        try:
            while not self.state.stop_requested:
                ...
                if stage not in ("PULLBACK", "MOMENTUM") or trend not in ("UP", "DOWN"):
                    self._prefetched_proposal = None
                    await asyncio.sleep(0.5)
                    continue
                ...
                try:
                    ...
                    if (
                        ...
                        and latest_state.get("pattern_stage") in ("PULLBACK", "MOMENTUM")
    ...
    async def _take_profit_loop(self, contract_id: int, buy_price: float):
        """Background loop that sets a take-profit limit order via Deriv contract_update.

        On a 5-tick ONETOUCH contract the monitoring window is extremely tight,
        so we set the TP limit order as soon as possible after the contract is
        purchased.  Deriv will auto-close the contract if the target profit is
        reached before expiry.
        """
        try:
            if not self._take_profit_enabled or self._take_profit_amount <= 0:
                return

            target_payout = buy_price + self._take_profit_amount
            logger.info(
                "Setting take-profit limit order on contract %s: buy=%.2f -> target=%.2f",
                contract_id, buy_price, target_payout,
            )

            await self._client.update_contract(
                contract_id=contract_id,
                take_profit=target_payout,
            )
            logger.info(
                "Take-profit limit order set successfully on contract %s (target: %.2f).",
                contract_id, target_payout,
            )
            self.state.update_strategy_state(take_profit_active=True)

        except DerivAPIError as exc:
            logger.warning(
                "Failed to set take-profit limit order on contract %s: %s",
                contract_id, exc,
            )
        except Exception as exc:
            logger.exception(
                "Unexpected error setting take-profit on contract %s: %s",
                contract_id, exc,
            )
