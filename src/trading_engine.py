...
No barriers. Contract length is passed into the strategy for duration-aware
entry analysis. Every terminal trade state is written to the journal with a
note explaining why.

New this round (additive only, no entry/exit logic changed): while a contract
is open the engine tracks the best/worst quote seen, and at settlement records
MAE / MFE (max adverse / favourable excursion) so the Performance Scope can
explain *why* a trade won or lost. Wrapped defensively so a tracking fault can
never affect the trade.
"""
import asyncio
import time
import uuid
...
class TradingEngine:
    def __init__(
        ...
        # Excursion tracking (additive; read by the journal/Scope only).
    ...
    def _mae_mfe(self, signal: str) -> Tuple[str, str]:
        """Return (mae, mfe) as formatted strings, or ('','') if unavailable."""
    ...
    async def _validate_symbol(self) -> bool:
        ...
        if self.symbol in all_syms:
            ...
            return True
        logger.warning("Symbol %s not listed in the account catalogue (%d returned). Proceeding. Sample: %s",
                ...
                if self._trade_in_progress:
                    ...
                    if busy_sid:
                        self._journal.record_outcome(
                            ...
                            0, note="a trade was already open in this tab")
                    continue
    ...
    async def _on_tick(self, tick_data: Dict[str, Any]):
        try:
            ...
            # Excursion tracking for the open contract (defensive; journal-only).
    ...
    async def _execute_trade(self, signal: str, entry_price: float, signal_id: Optional[str] = None):
        ...
        try:
            ...
            try:
                ...
                # Seed excursion tracking from the entry snapshot; ticks refine it.
                ...
                if self._active_contract_id is not None:
                    ...
                    self._journal.record_outcome(
                        signal_id, "CANCELLED", 0.0, stake, None, self.execution_mode, martingale_step,
                        note="Order attempt was stopped before any contract was confirmed.")
                raise
            except Exception as e:
                if self._active_contract_id is not None:
