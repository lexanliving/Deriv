...
Hardened async trading engine.

Key behavior:
- Fresh-signal trade gate with startup warm-up and post-cooldown grace.
- Version-aware strategy state pushing to reduce dashboard churn.
- Safe reconnect handling and ambiguous-buy reconciliation.
- Proposal prefetch loop and safe contract monitoring.
- Symbol is taken as a parameter and sent to Deriv EXACTLY as given
  (Deriv symbols are case-sensitive — never uppercased here).
"""
    ...
    async def _validate_symbol(self) -> bool:
        ...
        if match is None:
            examples = sorted({
                str(item.get("symbol", ""))
                for item in active_symbols
                if str(item.get("symbol", "")).startswith("1HZ")
            })[:10]
            msg = (
        ...
        if not bool(match.get("exchange_is_open", True)):
            logger.warning("Exchange for %s may be closed; attempting anyway.", self.symbol)
    ...
    async def _refresh_mtf(self):
        ...
        try:
            ...
            self.state.update_strategy_state(
                mtf_bias=bias, mtf_agreement=agreement, mtf_tf_biases=tf_biases
            )
