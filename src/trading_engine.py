...
Candle-trend engine for CALL/PUT (UP/DOWN) on forex & gold.

- Symbol is taken as a parameter and sent to Deriv exactly as given (case-sensitive).
- Symbol validation is OPPORTUNISTIC: the Deriv Options websocket returns an empty
  active_symbols catalogue for some account types, so the catalogue is a hint, never
  a gate. The tick subscription and the proposal are the source of truth; if a symbol
  truly cannot trade, Deriv's own error surfaces there.
- No proposal prefetch (minute contracts do not need it; fewer failure modes).
- No barriers: get_proposal is called with barrier=None.
"""
...
class TradingEngine:
    ...
    # ------------------------------------------------------------------
    # Symbol check — a hint, never a gate
    # ------------------------------------------------------------------
    async def _validate_symbol(self) -> bool:
        """Opportunistic check that NEVER hard-blocks.

        The Deriv Options websocket returns an empty active_symbols catalogue for
        some account types (notably MT5-style logins), even when tick streams or
        proposals still work. So an empty/missing catalogue, or a symbol not listed,
        only logs diagnostics and sets a calm status. The tick subscription and the
        proposal that follow are the real source of truth.
        """
        try:
        ...
        if self.symbol in all_syms:
            ...
            if match is not None and bool(match.get("is_trading_suspended", False)):
                logger.warning("Symbol %s reports is_trading_suspended; proceeding anyway (may be transient).", self.symbol)
        ...
        # Catalogue exists but does not list our symbol. The Options catalogue can be
        # incomplete for the account type, so do NOT block. Log the real list and
        # proceed; the subscription/proposal gives the verdict.
        logger.warning(
            "Symbol %s not listed in the account catalogue (%d symbols returned). "
            "Proceeding anyway. Sample available: %s",
    ...
    async def run(self):
        ...
        # Opportunistic: logs diagnostics, sets a calm status, never blocks.
    ...
    async def _refresh_candles(self):
        ...
        try:
            ...
            # Feed the chart from candle closes too, so the price path shows even
            # when the live tick stream is unavailable for this symbol/account.
