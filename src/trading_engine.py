...
from src.strategy import StrategyEngine, MTFAnalyzer, compute_dynamic_barrier
...
from config import (
    ...
    DYNAMIC_BARRIER_ENABLED,
    DYNAMIC_BARRIER_WINDOW,
    DYNAMIC_BARRIER_MULTIPLIER,
    DYNAMIC_BARRIER_FLOOR,
    DYNAMIC_BARRIER_CEIL,
)
...
class TradingEngine:
    ...
    async def _on_tick(self, tick_data: Dict[str, Any]):
        ...
        try:
            ...
            # fired, submit the trade before doing any UI/state bookkeeping -
            # nothing sits between "signal detected" and "order submitted".
    ...
    def _compute_barrier(self, signal: str) -> str:
        """Return the barrier offset string to quote for this signal.

        Uses a volatility-scaled barrier (see strategy-improvement report,
        July 2026) when DYNAMIC_BARRIER_ENABLED and there's enough recent
        tick history; otherwise falls back to the static configured
        barrier, so a quiet start-of-session or a disabled flag never
        blocks a trade.
        """
        if DYNAMIC_BARRIER_ENABLED:
            recent_ticks = self.state.get_recent_ticks()
            dynamic_offset = compute_dynamic_barrier(
                recent_ticks,
                window=DYNAMIC_BARRIER_WINDOW,
                multiplier=DYNAMIC_BARRIER_MULTIPLIER,
                floor=DYNAMIC_BARRIER_FLOOR,
                ceil=DYNAMIC_BARRIER_CEIL,
            )
            if dynamic_offset is not None:
                offset_str = f"{dynamic_offset:.4f}"
                return f"+{offset_str}" if signal == "BUY" else f"-{offset_str}"
        return self.barrier_buy if signal == "BUY" else self.barrier_sell
    ...
    async def _execute_trade(self, signal: str, entry_price: float):
        ...
        if self.state.drawdown_limit_hit():
            logger.warning("Session drawdown limit hit. Skipping signal %s.", signal)
            self.state.set_status(
                "Session drawdown limit reached. No new trades will be placed this session."
            )
            return
        ...
        barrier = self._compute_barrier(signal)
