...
active_symbols catalogue for some account types, so the catalogue is a hint,
never a gate. The tick subscription and the proposal are the source of truth.
...
def normalize_account_type(account_type: str) -> str:
    normalized = " ".join(
        str(account_type or "").strip().upper().replace("-", " ").split()
    )
