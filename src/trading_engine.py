...
# ---------------------------------------------------------------------------
# Trade-trigger safety gate
# ---------------------------------------------------------------------------
# The strategy is allowed to produce signals quickly, but the engine must not
# act on a signal that was generated before the bot was ready or before the
# sniper pacing cooldown expired. These constants force the engine to wait for
# a fresh signal after start-up and after every cooldown.
INITIAL_WARMUP_COOLDOWN_SECONDS = 30.0
POST_COOLDOWN_FRESH_SIGNAL_GRACE_SECONDS = 5.0
...
class TradingEngine:
    ...
    def __init__(
        ...
        self._strategy = StrategyEngine(
            ...
            entry_score_threshold=preset.get(
        ...
        # Trade-trigger safety gate. The gate does not change strategy signals;
        # it only prevents execution on stale/start-up/pre-cooldown signals.
    ...
    async def run(self):
        ...
        """
        logger.info(
            "Trading engine starting with execution mode %s for %s account.",
            self.execution_mode,
            self.account_type,
        )
        ...
        if self.execution_mode == "BLOCKED":
            ...
            self.state.set_status(
                "Signal monitoring is active, but no orders can be sent."
            )
        else:
            self.state.set_status(
                f"Connecting to Deriv API for {self.execution_mode.lower()} order execution..."
            )
        ...
        # Arm the trade gate only after the initial MTF data has been fetched.
        # Signals before this point, and for INITIAL_WARMUP_COOLDOWN_SECONDS
        # afterwards, are observed but not executed.
        ...
        self.state.set_status(
            f"Subscribed to tick stream. Warming up for "
            f"{INITIAL_WARMUP_COOLDOWN_SECONDS:.0f}s before the first trade..."
        )
        ...
        try:
            ...
            while not self.state.stop_requested:
                ...
                # Smart MTF Refresh
                # Refresh more frequently if the bot is in an active setup.
                strategy_state = self._strategy.get_state()
                active_setup = strategy_state.get("pattern_stage") in (
                    "TREND",
                    "PULLBACK",
                    "MOMENTUM",
                )
                current_refresh_interval = (
                    10.0 if active_setup else MTF_REFRESH_INTERVAL
                )
        ...
        async with self._reconnect_lock:
            ...
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    ...
                    logger.debug(
                        "Error while clearing the old Deriv socket.",
                        exc_info=True,
                    )
                ...
                try:
                    if await self._client.connect():
                        ...
                        # Require a fresh signal after reconnect; any signal that
                        # existed before the socket dropped is stale.
                        self._gate_ready_monotonic = None
                        self._last_signal_monotonic = 0.0
    ...
    def _update_trade_gate(self, now_mono: float) -> None:
        """Track when the engine is allowed to consume a signal.

        The gate is closed during initial warm-up and while StateManager
        pacing cooldown is active. When both are clear, the gate opens once
        and records the time. A trade is only allowed after a signal has been
        generated after that open time (plus a small grace period).
        """
        ...
        warmup_active = (
            now_mono - self._engine_ready_monotonic
        ) < INITIAL_WARMUP_COOLDOWN_SECONDS
        ...
        if warmup_active or cooldown_remaining > 0:
            ...
            logger.info(
                "Trade gate open: waiting for a fresh signal before executing."
            )

    def _trade_gate_allows_trade(self, now_mono: float) -> Tuple[bool, str]:
        """Return (allowed, reason). Must be called after a BUY/SELL signal."""
        if self._engine_ready_monotonic == 0.0:
            return False, "engine not ready"

        warmup_remaining = INITIAL_WARMUP_COOLDOWN_SECONDS - (
            now_mono - self._engine_ready_monotonic
        )
        ...
        grace_deadline = (
            self._gate_ready_monotonic
            + POST_COOLDOWN_FRESH_SIGNAL_GRACE_SECONDS
        )
        ...
        if self._last_signal_monotonic < grace_deadline:
            ...
            return (
                False,
                f"waiting for a fresh post-cooldown signal "
                f"({remaining_grace:.1f}s)",
            )
    ...
    async def _on_tick(self, tick_data: Dict[str, Any]):
        ...
        try:
            ...
            # runs, regardless of sniper pacing below. Pacing controls whether
            # a signal is ACTED ON, not whether the engine keeps analysing ticks.
            signal = self._strategy.process_tick(price)

            # Priority: evaluate -> send order immediately. If a signal
            # fired and the trade gate allows it, submit the trade before
            # doing any UI/state bookkeeping.
            if signal in ("BUY", "SELL"):
                ...
                if allowed:
                    ...
                    # A trade attempt will now start. The next trade must wait
                    # for a fresh signal after the new pacing cooldown.
                    self._gate_ready_monotonic = None
                    self._last_signal_monotonic = 0.0

                    self.state.set_status(
                        f"Signal: {signal} at {price:.4f}. Placing trade..."
                    )
                    ...
                    # Signal quality gate passed but the safety gate says wait.
                    # Release the pattern state back to IDLE so the engine looks
                    # for the next setup instead of staying wedged on this one.
                    logger.info(
                        f"Signal {signal} at {price} seen but not executed: {gate_reason}"
                    )
                    self.state.set_status(
                        f"Signal {signal} seen, waiting: {gate_reason}"
                    )
                    self._strategy.on_signal_skipped()

            elif signal == "PRE_FETCH":
                logger.debug(
                    "Pre-fetch signal received. Background loop will handle proposal request."
                )
            ...
            self.state.update_tick_and_strategy_state(
                ...
                last_signal_score_breakdown=strategy_state[
                    "last_signal_score_breakdown"
                ],
            )
        ...
        try:
            for tf_label, granularity in MTF_GRANULARITIES.items():
                candles = await self._client.get_candles(
                    SYMBOL, granularity, MTF_CANDLE_COUNT
                )
                candles_by_tf[tf_label] = candles
                logger.debug(f"MTF {tf_label}: {len(candles)} candles fetched.")

            bias, agreement, tf_biases = self._mtf_analyzer.analyze_with_strength(
                candles_by_tf
            )
            ...
            logger.info(
                f"MTF analysis complete. Bias: {bias_str} | TF: {tf_biases}"
            )
    ...
    async def _execute_trade(self, signal: str, entry_price: float):
        ...
        if self.execution_mode == "BLOCKED":
            ...
            self.state.set_status(
                f"Signal: {signal} at {entry_price:.4f}. Order blocked by safety gate."
            )
        ...
        if not self._client.connected:
            logger.warning(
                "Connection was down when signal fired. Reconnecting before placing the order..."
            )

            if not await self._reconnect():
                reason = (
                    "Order not sent: the Deriv connection could not be restored "
                    "before the proposal request."
                )
                self.state.update_trade_outcome(trade_id, "CANCELLED", 0.0, reason)
                self.state.set_error(reason)
                self.state.set_status(
                    "Order cancelled: could not reach Deriv before requesting a proposal."
                )
                self._trade_in_progress = False
                return
        ...
        try:
            ...
            self.state.set_status(
                f"{self.execution_mode} order: preparing to buy {signal} "
                f"(stake {stake:.2f} {self.account_currency})..."
            )
            ...
            # Step 2: Submit the buy request IMMEDIATELY.
            ...
            logger.info(
                "Proposal %s received for TradeID=%s; submitting buy request.",
                proposal_id,
                trade_id,
            )
            ...
            try:
                ...
                if buy_exc.code in ("TIMEOUT", "CONNECTION_LOST", "INVALID_RESPONSE"):
                    ...
                    if buy_response is None:
                        raise DerivAPIError(
                            "The buy request may have reached Deriv, but no receipt "
                            "or matching open contract could be confirmed. "
                            "Check the Deriv statement before restarting.",
                            "AMBIGUOUS_BUY",
                        ) from buy_exc
                else:
                    raise
            ...
            self.state.set_status(
                f"{self.execution_mode} contract {contract_id} active | "
                f"Waiting for Deriv settlement..."
            )

            # Step 3: Monitor contract until settlement
            outcome, pnl = await self._monitor_contract(
            ...
            if outcome == "UNKNOWN":
                reason = (
                    f"Contract {contract_id} was bought but its settlement was not "
                    "confirmed within the monitoring window. "
                ...
                self.state.set_error(reason)
                self.state.set_status(
                    f"Trading stopped: contract {contract_id} outcome is unresolved; "
                    "check Deriv before restarting."
                )
                ...
                logger.warning(
                    "Trade outcome unresolved | TradeID=%s | Contract=%s",
                    trade_id,
                    contract_id,
                )
                ...
                if outcome == "WON":
                    ...
                    self.state.on_trade_loss(
                        self.martingale_multiplier, self.max_martingale_steps
                    )
            ...
            if e.code == "AMBIGUOUS_BUY":
                ...
                self.state.set_error(reason)
                self.state.set_status(
                    "Trading stopped: a buy outcome is ambiguous. "
                    "Check the Deriv statement before restarting."
                )
            ...
            if self._active_contract_id is not None:
                reason = (
                    f"Unexpected monitoring failure for purchased contract "
                    f"{self._active_contract_id}: {e}. "
                    "Check the Deriv statement; Martingale was not changed."
                )
    ...
    async def _pre_fetch_loop(self):
        ...
        try:
            while not self.state.stop_requested:
                ...
                if stage not in ("TREND", "PULLBACK", "MOMENTUM") or trend not in (
                    "UP",
                    "DOWN",
                ):
                    self._prefetched_proposal = None
                ...
                try:
                    ...
                    if (
                        ...
                        and latest_state.get("pattern_stage")
                        in ("TREND", "PULLBACK", "MOMENTUM")
    ...
    async def _monitor_contract(
        ...
        logger.warning(
            f"Contract {contract_id} monitoring timed out without a confirmed outcome."
        )
