...
- At startup the engine validates the symbol against the account's live symbol list
  and, if it is missing, reports the symbols that ARE available (no more "none returned").
...
# Prefixes we recognise when building a helpful "available symbols" list.
_KNOWN_PREFIXES = ("frx", "1HZ", "R_", "BOOM", "CRASH", "WL", "JD", "OTC")
...
class TradingEngine:
    ...
    async def _validate_symbol(self) -> bool:
        try:
            ...
            return True  # cannot validate -> do not block; a bad pick will fail at proposal
        ...
        # Not available: build an actionable list of what IS available.
        known = [s for s in all_syms if s.startswith(_KNOWN_PREFIXES)]
        if not known:
            known = all_syms[:30]
        examples = sorted(set(known))[:30]
        msg = (
            f"Symbol '{self.symbol}' is not available for this account. "
            f"Available symbols include: {', '.join(examples) if examples else 'none returned by Deriv'}."
        )
        self.state.set_error(msg)
        self.state.set_status("Pick an available market from the list above.")
        return False
    ...
    async def _execute_trade(self, signal: str, entry_price: float):
        ...
        try:
            ...
            self.state.set_status(
                f"{self.execution_mode} contract {contract_id} active | Waiting for settlement..."
            )
