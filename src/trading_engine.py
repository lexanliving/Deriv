...
class TradingEngine:
    ...
    async def run(self):
        ...
        try:
            ...
            while not self.state.stop_requested:
                ...
                active_setup = strategy_state.get("pattern_stage") in ("PULLBACK", "MOMENTUM")
                current_refresh_interval = 15.0 if active_setup else MTF_REFRESH_INTERVAL
    ...
    async def _on_tick(self, tick_data: Dict[str, Any]):
        ...
        try:
            ...
            # Sniper Pacing and Quality Checks
            if not self.state.can_trade():
                # Ticks are still processed by the strategy to keep buffers fresh,
                # but we ignore signals until the cooldown period is over.
                strategy_state = self._strategy.get_state()
            ...
            if signal in ("BUY", "SELL"):
                ...
                # Update consecutive loss counter for pacing
                # This will be updated properly once the trade settles, 
                # but we track the attempt here to ensure pacing starts immediately
                return
