...
_SCHEMA = (
    "entry_analysis, exit_analysis, strategy_adherence, market_behaviour, confidence (0-100), "
    "strengths[], weaknesses[], mistakes[], pattern_detected, risk_observations[], "
    "suggested_improvements[], technical_explanation, ai_summary"
)
...
def _fallback_record(ctx):
    ...
    adherence = (
        f"All hard gates passed and score {score} met threshold {thr}."
        if (score is not None and thr is not None and score >= thr)
        else "Setup did not clear the live threshold."
    )

    return {
        "entry_analysis": f"Strongest: {', '.join(strong) or 'n/a'}. Weakest: {', '.join(weak) or 'n/a'}.",
        ...
        "weaknesses": weak,
        "mistakes": (
            ["duration too long for the move"]
            if (outcome == "LOST" and mfe is not None and mae is not None and mfe > mae * 0.5)
            else []
        ),
        "pattern_detected": ctx.get("regime"),
        "risk_observations": (
            [f"martingale step {ctx.get('martingale_step')}"]
            if ctx.get("martingale_step")
            else []
        ),
...
def generate_research(ctx, prior_knowledge, prior_research) -> Tuple[Dict[str, Any], str]:
    ...
    prompt = (
        "You are a senior quant researcher for a Deriv binary-options trend bot. "
        "Compare the NEW trade with PRIOR KNOWLEDGE and PRIOR RESEARCH before concluding. "
        "Answer ONLY JSON with keys: " + _SCHEMA + ".\n\n"
        f"NEW TRADE: symbol={ctx.get('symbol')} direction={ctx.get('direction')} "
        f"outcome={ctx.get('outcome')} pnl={ctx.get('pnl')} score={ctx.get('score')}/{ctx.get('threshold')} "
        f"regime={ctx.get('regime')} mtf={ctx.get('tf_biases')} mae={ctx.get('mae')} mfe={ctx.get('mfe')} "
        f"martingale_step={ctx.get('martingale_step')} factor_profile={prof} strong={strong} weak={weak} "
        f"rejection={ctx.get('rejection') or 'none'}\n"
