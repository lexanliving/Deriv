...
class DerivAPIClient:
    ...
    async def update_contract(
        ...
        """Update an open contract's limit order (take profit / stop loss).

        Uses the Deriv `contract_update` endpoint to set limit orders on a
        live contract.  At least one of ``take_profit`` or ``stop_loss`` must
        be provided.
        """
        if isinstance(contract_id, bool) or not isinstance(contract_id, int):
            raise DerivAPIError("A numeric contract ID is required.", "INVALID_REQUEST")
        if take_profit is None and stop_loss is None:
            raise DerivAPIError(
                "At least one of take_profit or stop_loss must be set.",
                "INVALID_REQUEST",
            )
        ...
        return self._require_object(response, "contract_update")
