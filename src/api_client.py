...
The Personal Access Token is sent only to the Options REST API. Deriv returns
a short-lived, account-specific WebSocket URL used for market data and trading
requests. The PAT is never placed in a WebSocket message or URL.
"""
...
class DerivAPIClient:
    ...
    async def _websocket_url(self) -> str:
        ...
        status, payload = await self._rest_request(
            "POST", endpoint, self.api_token, self.app_id
        )
        if not 200 <= status < 300:
            raise DerivAPIError(
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._ping_task = asyncio.create_task(
                self._ping_loop(), name="deriv-api-ping"
            )
    ...
    def _dispatch_tick(self, tick: Dict[str, Any]) -> None:
        """Run callbacks outside the reader task so request responses can't deadlock."""
    ...
    async def _send_request(self, payload: Dict[str, Any], timeout: float = REQUEST_TIMEOUT) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(response, dict):
                raise DerivAPIError(
                    "Deriv returned an invalid response envelope.", "INVALID_RESPONSE"
                )
            return response
            ...
            self._connected = False
            raise DerivAPIError(
                "Deriv connection lost while sending the request.", "CONNECTION_LOST"
            ) from exc
    ...
    async def get_active_symbols(self) -> List[Dict[str, Any]]:
        """List symbols this account can trade, for validation and diagnostics."""
        response = await self._send_request({"active_symbols": "brief"})
        symbols = response.get("active_symbols")
        if not isinstance(symbols, list):
            raise DerivAPIError(
                "Deriv returned an invalid active_symbols response.", "INVALID_RESPONSE"
            )
