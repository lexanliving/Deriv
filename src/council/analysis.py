...
def build_snapshot(candles: Sequence[dict], ticks: Sequence[float],
                   hour_utc: int, direction: str) -> dict:
    ...
    e_fast = ind.ema(closes, 9); e_slow = ind.ema(closes, 21)
    slope_fast = ind.slope_pct(e_fast, 5); slope_slow = ind.slope_pct(e_slow, 10)
    ...
    for c in candles[-14:]:
        ...
        o = float(c.get("open") or 0); cl = float(c.get("close") or 0)
        rng = max(h - l, 1e-9)
        wicks.append((rng - abs(cl - o)) / rng)
    noise = ind.mean(wicks)

    n = 14
    path = sum((highs[i] - lows[i]) for i in range(len(closes) - n, len(closes)))
    net = abs(closes[-1] - closes[-n]) if len(closes) >= n else 0.0
    efficiency = (net / path) if path > 0 else 0.0

    o = float(candles[-1].get("open") or 0); cl = closes[-1]
    rng = max(highs[-1] - lows[-1], 1e-9)
    body = abs(cl - o) / rng
    aligned = (cl > o) if direction == "BUY" else (cl < o)
    candle_quality = body if aligned else body * 0.3

    swing_high = max(highs[-n:]); swing_low = min(lows[-n:])
    sr_dist = ((swing_high - cl) / atr_abs) if (direction == "BUY" and atr_abs) else \
              ((cl - swing_low) / atr_abs) if atr_abs else 0.0
    pull_dist = abs(cl - e_slow[-1]) / atr_abs if atr_abs else 0.0

    half = n
    if len(highs) >= 2 * half:
        prev_hh = max(highs[-2 * half:-half]); cur_hh = max(highs[-half:])
        prev_ll = min(lows[-2 * half:-half]); cur_ll = min(lows[-half:])
        if direction == "BUY":
    ...
    tick_noise = ind.stdev([ticks[i] - ticks[i - 1] for i in range(1, len(ticks))]) if len(ticks) > 2 else None
    session = 1.0 if 7 <= hour_utc <= 16 else 0.6

    snap = {"closes": closes, "e_fast": e_fast, "e_slow": e_slow, "slope_fast": slope_fast,
            "slope_slow": slope_slow, "vol_short": vol_short, "vol_long": vol_long,
            "vol_ratio_regime": vol_ratio_regime, "atr": atr, "rsi": rsi, "chop": chop,
            "noise": noise, "efficiency": efficiency, "candle_quality": candle_quality,
            "sr_dist": sr_dist, "pull_dist": pull_dist, "structure": structure,
            "vol_ratio": vol_ratio, "tick_noise": tick_noise, "session": session,
            "direction": direction}
