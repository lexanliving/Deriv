"""Recalibrated Council + telemetry. Hard/soft/informational tiers, volatility-
adaptive thresholds, calibration logging with symbol/direction/timestamp so the
UI can prove the Council is connected and reviewing."""
...
def review(setup: dict, state) -> dict:
    ...
    if len(candles) < 30:
        ...
        _record_cal({**out, "hard": [], "low_conf": False, "factors": []})
        return out
    ...
    if hard:
        ...
        out.update({"symbol": symbol, "direction": direction, "ts": now_iso})
        _record_cal({**out, "hard": names, "low_conf": False, "factors": []})
        return out
    ...
    out.update({"symbol": symbol, "direction": direction, "ts": now_iso})
    _record_cal({**out, "hard": [], "low_conf": outcome == "REJECT", "factors": factors})
    return out
