...
Thread-safe shared state for the trading engine (background thread) and the
Streamlit dashboard (main thread).

Hardening:
- Capped trade history (deque) + O(1) id index.
- Settled trades can never be updated twice (no double-counted P&L).
- Expectancy tracking: avg win, avg loss, expectancy per trade.
"""
...
@dataclass
class TradeRecord:
    """A single trade attempt and its outcome."""
    ...
    def update_trade_outcome(self, trade_id, status, pnl, error_message=""):
        with self._lock:
            ...
            if trade.status in FINAL_TRADE_STATUSES:
                return  # already settled — never double-count
