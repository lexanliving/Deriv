...
from config import TICK_BUFFER_SIZE, DEFAULT_INITIAL_STAKE, SESSION_MAX_DRAWDOWN, STAKE_MODE
...
class StateManager:
    ...
    def get_martingale_state(self) -> Dict[str, Any]:
        with self._lock:
            # In FLAT mode (the default) the stake never scales with losses -
            # step is still tracked/exposed for the UI, but stake stays flat.
            # See config.STAKE_MODE for the rationale.
            stake = self._current_stake if STAKE_MODE == "MARTINGALE" else self._initial_stake
            return {
    ...
    def drawdown_limit_hit(self) -> bool:
        """True once session P&L has fallen to the configured circuit-breaker
        level. SESSION_MAX_DRAWDOWN=None disables this check entirely."""
        if SESSION_MAX_DRAWDOWN is None:
            return False
        with self._lock:
            return self._total_pnl <= -abs(SESSION_MAX_DRAWDOWN)
