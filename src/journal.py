...
src/journal.py — decision + execution log with a crash-proof archive.
  ...
  logs/trade_journal.csv   — live working file: one row per 15m review (with the
                             symbol, signal id, regime, duration and the full
                             confluence breakdown), outcome + MAE/MFE back-filled
                             in place. Shown in the Terminal preview and offered
                             as the CSV download.
...
New this round (additive only): columns regime, duration_min, mae, mfe. Old
files are migrated in place by column name, so rows never misalign. Every
operation is defensive — a journal failure can never interrupt trading.
"""
import csv
import os
import threading
...
COLUMNS = [
    ...
    "regime", "duration_min", "mae", "mfe",
]
ARCHIVE_COLUMNS = ["kind"] + COLUMNS
# Outcome events only carry these; on merge they overwrite the EVAL row's blanks.
...
class TradeJournal:
    ...
    def record_outcome(self, signal_id, outcome, pnl, stake, contract_id,
                       execution_mode, martingale_step="", note="",
                       mae="", mfe="") -> None:
        if not signal_id:
            return
        try:
