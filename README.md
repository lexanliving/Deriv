...
After arming, the bot waits for the configured lower-digit confirmation sequence from 0 through 6. The final required lower digit itself immediately queues the `DIGITOVER` contract with barrier `6`; there is no extra higher-digit gap. The default remains the original **one lower tick as the entry trigger**. Optional two- or three-lower-tick timing requires consecutive lower digits; any 7–9 digit before completion resets that sequence and does not trigger a trade. This timing gate does not replace or change the concentration rule. When a contract finishes, the bot re-arms only after the result is finalized and only if the latest minute-reviewed condition is still valid. A later invalid review disarms it. The selected duration is one or two ticks; a two-tick contract settles on its final tick and is not two independent attempts.
...
The first recommended run is monitoring or demo mode. The dashboard makes the 31% share, exact counts, per-digit averages, lower-confirmation count, recovery steps, and positive take-profit target explicit. The journal records every minute review, rolling digit counts, lower-tick confirmation sequence, proposal ask/payout, execution result, and settlement outcome in the `logs` directory. Coordination state files in that directory are local runtime safeguards; if a bought contract becomes orphaned or its settlement is unknown, the market is blocked for manual statement verification.
