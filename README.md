...
After arming, the bot waits for the configured lower-digit confirmation sequence from 0 through 6. **The minute review only arms the condition; it never counts as a lower confirmation.** The sequence starts strictly after that review boundary, and delayed/pre-review ticks are ignored for confirmation. For example, with `N=3`, a review at `38:00` permits `38:01=1/3`, `38:02=2/3`, and `38:03=3/3`; the `38:03` digit itself immediately queues the `DIGITOVER` contract with barrier `6`. There is no extra higher-digit gap. The default remains the original **one lower tick as the entry trigger**. Optional two- or three-lower-tick timing requires consecutive lower digits; any 7–9 digit before completion resets that sequence and does not trigger a trade. Each new qualifying minute review resets any partial sequence and establishes a new boundary. This timing gate does not replace or change the concentration rule. When a contract finishes, the bot re-arms only after the result is finalized and only if the latest minute-reviewed condition is still valid; a fresh sequence then begins after the new post-settlement boundary. A later invalid review disarms it. The selected duration is one or two ticks; a two-tick contract settles on its final tick and is not two independent attempts.
