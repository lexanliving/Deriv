"""pages/venture.py — Venture Advisory page (auto-discovered)."""
import os, sys
...
st.caption("The AI panel re-scans stored research every few minutes and advises whether it's a good venture and how much to risk. Read-only; the gate applies it automatically.")
...
st.markdown(f'<div class="va-card"><div class="va-verdict {vcls}">{verdict}</div>'
  f'<div style="margin-top:6px;font-size:.85rem;">Risk multiplier <b>{mult:.2f}</b> · confidence {advice.get("confidence",0)} · period {advice.get("period_days",30)}d · updated {advice.get("created_at") or "—"}</div>'
  f'<div style="margin-top:10px;font-size:.85rem;">{advice.get("reasoning","")}</div></div>', unsafe_allow_html=True)
...
st.caption("Verdict POOR blocks new entries; the multiplier scales stake. Offline/AI-down ⇒ neutral, multiplier 1.0.")
