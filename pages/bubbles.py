...
Duration-aware: the trigger timeframe shown in every narrative is derived from
the trade's duration (5m/15m -> 5m trigger, 30m/60m -> 15m trigger).
"""
...
# ---------------------------------------------------------------------------
COMP = [
    ("trend", 5), ("trigger", 3), ("momentum", 3), ("volatility", 2),
...
# Duration-aware trigger timeframe (mirrors config.ENTRY_TIMEFRAME_BY_DURATION).
ENTRY_TF_BY_DURATION = {
    5: "5m",
    15: "5m",
    30: "15m",
    60: "15m",
...
# ---------------------------------------------------------------------------
# Helpers — ALL defined before the tabs that call them.
# ---------------------------------------------------------------------------
def _g(d, k):
    v = d.get(k)
...
def _read_row(l, v, i):
    return (
        f'<div class="read"><span class="read-l">{l}</span>'
...
def _render_trade_card(t):
    ...
    tf = t.get("tf") or {}
    tf_html = (
        _tf_chip("5m", tf.get("5m")) + _tf_chip("15m", tf.get("15m"))
    ...
    with left:
        st.markdown(
            '<div class="panel"><div class="sec-h">BEFORE · why the engine took / passed it</div>',
            unsafe_allow_html=True,
        )
    ...
    with right:
        st.markdown(
            '<div class="panel"><div class="sec-h">AFTER · what happened &amp; why</div>',
            unsafe_allow_html=True,
        )
...
# ---------------------------------------------------------------------------
# Sidebar (no rows needed — always renders, always interactive)
# ---------------------------------------------------------------------------
with st.sidebar:
    st.markdown(
    ...
    start_bal = st.number_input(
        "Starting balance (for equity curve)", 0.0, 10000000.0, 0.0, 100.0, format="%.2f",
...
# ---------------------------------------------------------------------------
# Top bar — selectors live here, OUTSIDE the heavy compute
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Masked sets + KPIs (safe defaults first, then fill inside try)
# ---------------------------------------------------------------------------
...
try:
    mrows = [
        t for t in rows
        if (sym_filter == "All markets" or t["sym"] == sym_filter)
        and _period_mask(t, period, now)
    ]
...
with tab_ov:
    try:
        ...
        with g2:
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Monthly performance</span></div>'
                '<div id="mo"></div></div>',
                unsafe_allow_html=True,
            )
        ...
        with c1:
            ...
            with colh[1]:
                st.markdown(
                    ...
                    f'{cal_mod.month_name[m]} {y}</span></div>',
                    unsafe_allow_html=True,
                )
        ...
        with c3:
            ...
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Asset performance</span></div>',
                unsafe_allow_html=True,
            )
            ...
            if sym_items:
                ...
                fig.update_layout(
                    **PLOT(150), showlegend=False,
                    annotations=[dict(
                        text=f"<b>{money(net)}</b>", x=0.5, y=0.5,
        ...
        with r1:
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Recent trades</span></div>',
                unsafe_allow_html=True,
            )
        ...
        with r2:
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Streaks</span></div>'
                '<div class="streaks">'
...
with tab_cal:
    try:
        ...
        with h4:
            ...
            cur_label = next(
                (o for o in day_opts if o.startswith((st.session_state.scope_day or "######"))),
                "— none —",
            )
            pick = st.selectbox(
                "Jump to day", day_opts,
    ...
    try:
        ...
        with g3a:
            ...
            if hr:
                ...
                st.markdown(
                    '<div class="mut" style="padding:14px 0;text-align:center">No data.</div>',
                    unsafe_allow_html=True,
                )
        ...
        if weeks:
            for (yr, wk) in sorted(weeks.keys(), reverse=True):
                ...
                if top_rej:
                    hints.append(
                        f"Most stand-asides came from <b>{top_rej}</b> — if that's 'no trigger' the market was ranging (normal); "
