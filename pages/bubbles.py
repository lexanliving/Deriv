...
animation:ps-scan 7s linear infinite;
border-radius:2px;
}
...
animation:ps-sheen 8s linear infinite;
}
...
display:grid;
place-items:center;
background:linear-gradient(150deg,#0e2036,#0b1626);
...
@keyframes ps-pulse{
0%,100%{box-shadow:0 0 3px rgba(16,185,129,.5);}
50%{box-shadow:0 0 12px rgba(16,185,129,.95);}
}
...
display:grid;
place-items:center;
background:var(--ib,rgba(51,80,126,.18));
color:var(--ac,#7fa6e0);
font-size:.95rem;
}
...
outline-offset:-1px;
box-shadow:0 0 0 3px rgba(56,132,255,.18);
}
...
color:#7fb0ff;
border:1px solid rgba(56,132,255,.3);
}
...
border:1px solid rgba(56,132,255,.22);
font-size:.78rem;
color:#bcd2f5;
line-height:1.5;
}
...
ENTRY_TF_BY_DURATION = {1: "5m", 2: "5m", 5: "5m", 15: "5m", 30: "15m", 60: "15m"}
...
def kpi(label, value, sub, icon, ac, vc, ib, delay):
    return (
        f'<div class="ps-kpi" style="--ac:{ac};--vc:{vc};--ib:{ib};animation-delay:{delay}ms">'
...
def _tab_error(where, exc):
    st.markdown(
        ...
        f"The filters above stay live. Details in the expander.</div></div>",
        unsafe_allow_html=True,
    )
...
def _comp_bar(name, val, mx):
    ...
    return (
        f'<div class="cb"><div class="cb-n">{name}</div>'
...
def _read_row(l, v, i):
    return f'<div class="read"><span class="read-l">{l}</span><span class="read-v">{v}</span><span class="read-i">{i}</span></div>'
...
def _entry_narrative(t):
    ...
    b.append(
        f'<li><span class="gt">Higher-timeframe trend:</span> the 30m and 1h candles agreed on '
    ...
    if ts >= 3:
        b.append(
            f'<li><span class="gt">Trigger:</span> the closed {entry_tf} candle broke the prior candle’s {hi} on a '
        ...
        b.append(
            f'<li><span class="bd">Trigger:</span> the {entry_tf} candle broke the prior {hi} but on a modest body — '
    ...
    if adx is not None:
        ...
        b.append(
            f'<li><span class="gt">Entry-timeframe trend:</span> {entry_tf} ADX <b>{adx:.1f}</b> ({strength}) — '
    ...
    if mc >= 2:
        b.append(
            f'<li><span class="gt">MACD:</span> histogram was aligned with the trade <b>and accelerating</b> '
...
def _exit_narrative(t):
    ...
    if oc == "WON":
        if has and mae > 0 and mfe > 0:
            lines.append(
                f"It drew down <b>{mae:.5f}</b> against you, then ran <b>{mfe:.5f}</b> in your favour and "
                f"closed in profit — the setup had to hold through a scare, and it did. Conviction paid."
            )
        elif has:
            lines.append(
                f"Clean run — price moved your way almost the whole time (MFE <b>{mfe:.5f}</b>, drawdown "
        ...
        if has and mfe > 0 and mfe > mae * 0.5:
            lines.append(
                f"<b>This is the instructive one.</b> Price WAS in profit (MFE <b>{mfe:.5f}</b>) but gave it "
            ...
            lines.append(
                f"Wrong from the start — price moved against you immediately (MAE <b>{mae:.5f}</b>, MFE only "
        ...
        lines.append(
            "Result was never confirmed by Deriv (MAE/MFE may be partial). The statement is the source of "
...
def _render_trade_card(t):
    ...
    tf_html = (
        _tf_chip("5m", tf.get("5m"))
        + _tf_chip("15m", tf.get("15m"))
        + _tf_chip("30m", tf.get("30m"))
        + _tf_chip("1h", tf.get("1h"))
    )
    ...
    with left:
        ...
        if taken:
            ...
            st.markdown(
                f'<div class="aside"><span class="aside-t">STOOD ASIDE</span>'
...
with st.sidebar:
    st.markdown(
        "<div style='font-family:Space Grotesk;font-size:.74rem;color:#6b7c97;font-weight:600;"
...
<div class="ps-topbar">
  <div class="ps-brand">
    ...
    <div>
      <h1>Performance Scope</h1>
      <div class="sub">Trading Journal · MomentumMaster TF</div>
      <div class="ps-live"><span class="dot"></span>{n_reviews} reviews on record</div>
    </div>
  </div>
...
if not rows:
    st.markdown(
        '<div class="ps-panel"><div class="ps-empty"><div class="e">No journal data yet</div>'
        ...
        "here, and the cockpit fills in automatically.</div></div></div>",
        unsafe_allow_html=True,
    )
...
with tab_ov:
    try:
        ...
        net_ac = "#22c55e" if net >= 0 else "#ef4444"
        net_vc = "#4ade80" if net >= 0 else "#fb7185"
        net_ib = "rgba(34,197,94,.16)" if net >= 0 else "rgba(239,68,68,.16)"

        wr_ac = "#22c55e" if wr >= 55 else ("#ef4444" if execd and wr < 45 else "#3884ff")

        kpis = (
            kpi("Net P&L", money(net), f"equity {money(equity_now, False)}", "₿", net_ac, net_vc, net_ib, 0)
            + kpi("Win rate", pct(wr), f"{len(wins)}W · {len(losses)}L", "◎", wr_ac, "#eef3fb", "rgba(56,132,255,.16)", 40)
            + kpi(
                "Profit factor",
                f"{pf:.2f}" if pf != float("inf") else "∞",
                f"edge {money(expect)} / trade",
                "⚡",
                "#a855f7",
                "#eef3fb",
                "rgba(168,85,247,.16)",
            ...
            + kpi("Max drawdown", money(max_dd), f"risk/reward {rr:.2f}", "↯", "#ef4444", "#fb7185", "rgba(239,68,68,.16)", 120)
        )
        ...
        with g1:
            ...
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Equity curve</span>'
                '<span style="display:flex;align-items:center;gap:10px">'
                '<span class="ps-now"><span class="ps-now-dot"></span>live</span>'
                f'<span class="big">{money(equity_now, False)}</span>'
                f'<span class="delta {net_cls}">{money(net)}</span></span></div>'
                '<div id="eq"></div></div>',
                unsafe_allow_html=True,
            )
        ...
        with g2:
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Monthly performance</span></div><div id="mo"></div></div>',
                unsafe_allow_html=True,
            )
            ...
            if months_present:
                seq = _month_seq(
                    tuple(map(int, months_present[0].split("-"))),
                    tuple(map(int, months_present[-1].split("-"))),
                )
                ...
                fig = go.Figure(
                    go.Bar(
                        x=mx,
                        y=my,
                        marker_color=["#22c55e" if v >= 0 else "#ef4444" for v in my],
        ...
        with c1:
            ...
            with colh[1]:
                st.markdown(
                    f'<div class="ps-panel-h" style="justify-content:center;margin:0">'
                    f'<span style="color:#eef3fb;font-size:.9rem;letter-spacing:.05em">{cal_mod.month_name[m]} {y}</span></div>',
                    unsafe_allow_html=True,
                )
            ...
            st.markdown(
                '<div class="ps-panel" style="margin-top:8px">'
                + _cal_html(y, m, month_days, st.session_state.scope_day, scale)
                + "</div>",
                unsafe_allow_html=True,
            )
        ...
        with c2:
            ...
            mpnl_cls = "pos" if mpnl >= 0 else "neg"
            mret_cls = "pos" if mret >= 0 else "neg"

            st.markdown(
                f'<div class="ps-panel"><div class="ps-panel-h"><span>{cal_mod.month_name[m]} {y}</span>'
                '<span class="chip win">closed</span></div>'
                f'<div class="ps-panel-h" style="margin:0 0 12px 0"><span class="big {mpnl_cls}">{money(mpnl)}</span>'
                f'<span class="delta {mret_cls}">{mret:+.2f}%</span></div>'
                '<div class="meta" style="display:flex;flex-direction:column;gap:7px;font-family:JetBrains Mono,monospace;font-size:.74rem">'
                f'<div style="display:flex;justify-content:space-between"><span class="mut">Total trades</span><b>{mtrades}</b></div>'
                f'<div style="display:flex;justify-content:space-between"><span class="mut">Winning</span>'
                f'<b class="pos">{mw} ({(mw / mtrades * 100 if mtrades else 0):.0f}%)</b></div>'
                f'<div style="display:flex;justify-content:space-between"><span class="mut">Losing</span>'
                f'<b class="neg">{ml} ({(ml / mtrades * 100 if mtrades else 0):.0f}%)</b></div>'
        ...
        with c3:
            ...
            if sym_items:
                fig = go.Figure(
                    go.Pie(
                        values=[abs(v) for _, v in sym_items],
                        labels=[s for s, _ in sym_items],
                        hole=0.7,
                ...
                for (s, v), col in zip(sym_items, PALETTE):
                    leg += (
                        f'<div class="li"><span class="dot" style="background:{col}"></span>'
                ...
                st.markdown(
                    '<div class="mut" style="padding:20px 0;text-align:center">No trades yet.</div></div>',
                    unsafe_allow_html=True,
                )
        ...
        with r1:
            ...
            if recent:
                ...
                styled = df.style.map(
                    lambda v: "color:#4ade80;font-weight:700" if v == "WON" else ("color:#fb7185;font-weight:700" if v == "LOST" else ""),
                    subset=["Result"],
                ).map(
                    lambda v: "color:#4ade80;font-weight:700" if isinstance(v, float) and v > 0 else ("color:#fb7185;font-weight:700" if isinstance(v, float) and v < 0 else ""),
                    subset=["P&L"],
                ).format({"P&L": "{:+,.2f}"})
                ...
                st.markdown(
                    '<div class="mut" style="padding:20px 0;text-align:center">No closed trades in this window.</div>',
                    unsafe_allow_html=True,
                )
        ...
        with r2:
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Streaks</span></div><div class="streaks">'
...
with tab_cal:
    try:
        ...
        with h3:
            st.markdown(
                f'<div style="text-align:center;font-family:Space Grotesk,sans-serif;font-weight:700;color:#eef3fb;'
                f'font-size:1.05rem;letter-spacing:.05em;padding-top:6px">{cal_mod.month_name[m]} {y}</div>',
                unsafe_allow_html=True,
            )
        ...
        with h4:
            day_opts = ["— none —"] + [
                f"{d} ({month_days[d][0]:+,.2f} · {month_days[d][1]} trade{'s' if month_days[d][1] != 1 else ''})"
            ...
            pick = st.selectbox(
                "Jump to day",
                day_opts,
                index=day_opts.index(cur_label) if cur_label in day_opts else 0,
                key="cal_daypick",
            )
            ...
            if pick == "— none —":
                ...
                st.session_state.scope_day = pick.split(" ")[0]

        st.markdown(
            '<div class="ps-panel" style="margin-top:10px">'
            + _cal_html(y, m, month_days, st.session_state.scope_day, scale)
            + "</div>",
            unsafe_allow_html=True,
        )
        ...
        if st.session_state.scope_day:
            ...
            dpnl_cls = "pos" if dpnl >= 0 else "neg"

            st.markdown(
                f'<div class="ps-panel-h" style="margin:18px 0 4px 0"><span>{d} · {len(day_trades)} reviews</span>'
                f'<span class="delta {dpnl_cls}">{money(dpnl)}</span></div>',
                unsafe_allow_html=True,
            )
...
with tab_tr:
    try:
        ...
        pool = sorted(
            pool,
            key=lambda t: (t["ts"] if _valid(t) else pd.Timestamp.min.tz_localize("UTC")),
            reverse=True,
        )
...
with tab_an:
    try:
        st.caption(
            "Everything below is computed from your journal — the tuning loop. "
            "Read it weekly, change one thing on demo, then forward-test."
        )
        ...
        with a1:
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Rejection funnel — where setups die</span></div>',
                unsafe_allow_html=True,
            )
            ...
            if rej:
                ...
                for reason, c in top:
                    ...
                    bars += (
                        f'<div class="bar-row"><div class="nm" style="width:150px;font-size:.66rem">{reason[:34]}</div>'
                ...
                st.markdown(
                    '<div class="mut" style="padding:18px 0;text-align:center">No rejections in this window.</div></div>',
                    unsafe_allow_html=True,
                )

        with a2:
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Win rate by score bucket</span></div>',
                unsafe_allow_html=True,
            )
            ...
            if any(n for _, n, _ in rows_b):
                ...
                st.markdown(
                    '<div class="mut" style="padding:18px 0;text-align:center">No trades to bucket yet.</div></div>',
                    unsafe_allow_html=True,
                )

        st.markdown(
            '<div class="ps-panel" style="margin-top:14px"><div class="ps-panel-h">'
            "<span>Component edge — which factor separates winners from losers</span></div></div>",
            unsafe_allow_html=True,
        )
        ...
        if wins or losses:
            html_t = (
                '<table class="edge-tbl"><tr><th>factor</th><th>max</th><th>avg in wins</th>'
            ...
            for name, wa, la, edge in edge_rows:
                ...
                html_t += (
                    f"<tr><td>{name}</td><td class=\"num mut\">{COMP_MAX[name]}</td>"
            ...
            st.caption(
                "Positive edge = winners score higher on that factor (it's doing its job). "
                "Near-zero or negative edge = the factor isn't filtering — a candidate to re-weight or drop."
            )
        else:
            st.markdown(
                '<div class="mut" style="padding:14px 0;text-align:center">Need wins and losses to compute edge.</div>',
                unsafe_allow_html=True,
            )
        ...
        with g3a:
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Win rate by hour of day (UTC)</span></div>',
                unsafe_allow_html=True,
            )
        ...
        with g3b:
            st.markdown(
                '<div class="ps-panel"><div class="ps-panel-h"><span>Win rate by weekday</span></div>',
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            '<div class="ps-panel-h" style="margin:20px 0 6px 0"><span>Weekly reports — your edit-the-bot brief</span></div>',
            unsafe_allow_html=True,
        )
        ...
        if weeks:
            for (yr, wk) in sorted(weeks.keys(), reverse=True):
                ...
                if wwr < 50 and worst_name is not None and worst_edge < 0:
                    hints.append(
                        f"On losing setups this week the <b>{worst_name}</b> factor scored higher than on winners "
                        f"(edge {worst_edge:+.2f}) — it isn't filtering; consider raising its bar or trimming its weight."
                    )

                if top_rej:
                    hints.append(
                        f"Most stand-asides came from <b>{top_rej}</b> — if that's 'no trigger' the market was ranging "
                        f"(normal); if it's a score shortfall, the threshold may be a touch high for these conditions."
                    )
                ...
                wwr_cls = "pos" if wwr >= 50 else "neg"
                wpnl_cls = "pos" if wpnl >= 0 else "neg"

                st.markdown(
                    f'<div class="week"><div class="wh"><span class="wk">Week {wk} · {yr}</span>'
                    f'<span class="wm"><span>trades <b>{len(wex)}</b></span>'
                    f'<span>win rate <b class="{wwr_cls}">{wwr:.0f}%</b></span>'
                    f'<span>P&L <b class="{wpnl_cls}">{money(wpnl)}</b></span>'
