"""
config.py
MomentumMaster TF — multi-market candle-trend build.
Trades CALL/PUT (UP/DOWN) on forex & gold (and synthetics if the account has them).
No barriers: CALL/PUT contracts do not use them.
"""
...
# --- Markets ---
# Gold first because that is the intended default and the most widely available.
# frx* = forex/commodities (CALL/PUT). 1HZ* = synthetic 1s indices (only on some accounts).
# The dropdown shows all of these; the engine validates the pick against the live
# account symbol list at startup and reports what IS available if a pick is invalid.
AVAILABLE_MARKETS = {
...
# Barriers are not used by CALL/PUT. Kept empty so nothing imports a missing name.
BARRIER_BUY = ""
BARRIER_SELL = ""
