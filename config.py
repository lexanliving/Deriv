...
# ---------------------------------------------------------------------------
# v3: Multi-Factor Scoring Gate
# ---------------------------------------------------------------------------
# Every candidate signal is scored across four dimensions:
#   MTF Alignment (0-5 pts), Efficiency Ratio (0-4 pts),
#   Acceleration (0-3 pts), Continuation Strength (0-2 pts).
# A minimum composite score of ENTRY_SCORE_THRESHOLD (out of 14 possible)
# is required to fire a trade. This makes the engine 10x more selective
# than the previous binary pass/fail gate.
ENTRY_SCORE_THRESHOLD = 8      # Minimum composite score to enter a trade (out of 14)
...
MIN_TICK_VOLATILITY = 0.002    # Reject entries in a near-flat market (sniper filter)
MAX_TICK_VOLATILITY = 0.04     # Reject entries in an erratic/spiky market (sniper filter)
...
ACCELERATION_MIN_RATIO = 0.9   # Ensure momentum is not dying out before entry
...
CONTRACT_DURATION = 7           # Duration in ticks (more time for momentum to hit barrier)
CONTRACT_DURATION_UNIT = "t"    # 't' = ticks
BARRIER_BUY = "+0.05"           # Barrier offset for Buy (Touch above) - tighter for 1s index
BARRIER_SELL = "-0.05"          # Barrier offset for Sell (Touch below) - tighter for 1s index
CURRENCY = "USD"
...
STRATEGY_SENSITIVITY_PRESETS = {
    "Conservative": {
        ...
        "mtf_min_agreement": 3,
        "entry_score_threshold": 10,  # Very high bar: near-perfect setups only
    },
    "Balanced": {
        "velocity_threshold": VELOCITY_THRESHOLD,
        "burst_threshold": BURST_VELOCITY_THRESHOLD,
        "mtf_min_agreement": MTF_MIN_AGREEMENT,
        "entry_score_threshold": ENTRY_SCORE_THRESHOLD,  # Default: 8/14
    },
    "Aggressive": {
        "velocity_threshold": 0.50,
        "burst_threshold": 0.68,
        "mtf_min_agreement": 2,
        "entry_score_threshold": 6,  # Lower bar: more signals, less filtering
    },
}
