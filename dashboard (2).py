...
def _stretch(fn, *args, **kwargs):
    """Call a Streamlit element with full-width sizing, tolerant of API drift.

    Some Streamlit versions accept width='stretch', some only accept the
    older use_container_width=True, and at least one known point release
    raises instead of warning when width= is passed to certain elements.
    This tries the modern kwarg first and falls back automatically so a
    Streamlit version mismatch can't crash the app on a cosmetic sizing arg.
    """
    try:
...
with st.sidebar:
    ...
    with col_start:
        start_pressed = _stretch(st.button, "START", type="primary", disabled=state.is_running)
    with col_stop:
        stop_pressed = _stretch(st.button, "STOP", type="secondary", disabled=not state.is_running)
...
def ledger_fragment():
    ...
    _stretch(st.dataframe, styled, height=320, hide_index=True)
