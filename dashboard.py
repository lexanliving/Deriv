...
watchdog fragment auto-restarts the engine thread if it dies while the user
intends it to run (resuming martingale / P&L / daily cap). A Strategy Journal
section records every 15m decision and offers a CSV download for analysis.
"""
import asyncio
import html
import os
...
html, body, .stApp { background-color:#060912; color:#c7d2e0;
...
.mm-header { display:flex; align-items:flex-end; justify-content:space-between;
...
.mm-header::after { content:" "; position:absolute; left:0; right:0; bottom:0; height:2px;
  ...
  background-size:220% 100%; animation:mm-scan 6s linear infinite; border-radius:2px; }
@keyframes mm-scan { 0%{background-position:120% 0;} 100%{background-position:-120% 0;} }
.mm-logo { font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:1.32rem;
  letter-spacing:0.18em; color:#eef3fb; text-transform:uppercase; }
.mm-logo .mm-dot { color:#10b981; }
.mm-sub { font-family:'JetBrains Mono',monospace; font-size:0.7rem; color:#6b7c97;
...
.mm-dotlive { display:inline-block; width:8px; height:8px; border-radius:50%;
...
@keyframes mm-pulse { 0%,100%{box-shadow:0 0 4px rgba(16,185,129,0.5);}
  50%{box-shadow:0 0 14px rgba(16,185,129,0.95);} }

.mm-strip { padding:9px 15px; border-radius:9px; font-size:0.8rem; font-weight:500;
  margin:14px 0 18px 0; letter-spacing:0.01em; }
...
.mm-kpi { position:relative; background:linear-gradient(150deg,#0c1426,#0e1830);
  border:1px solid #1d2c49; border-radius:11px; padding:16px 18px 15px 18px; overflow:hidden;
  transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease; }
.mm-kpi:hover { transform:translateY(-3px); border-color:#33507e; box-shadow:0 10px 26px rgba(0,0,0,0.4); }
.mm-kpi::before { content:" "; position:absolute; top:0; left:0; right:0; height:3px;
  background:var(--accent,#33507e); }
.mm-kpi-hero { padding-top:20px; padding-bottom:20px; }
.mm-kpi__label { font-family:'Space Grotesk',sans-serif; font-size:0.62rem; font-weight:600;
  letter-spacing:0.16em; text-transform:uppercase; color:#6b7c97; }
.mm-kpi__value { font-family:'JetBrains Mono',monospace; font-weight:700; line-height:1.02;
  margin-top:8px; color:var(--val,#eef3fb); font-variant-numeric:tabular-nums; }
...
.mm-rail { background:linear-gradient(160deg,#0c1426,#0b1222); border:1px solid #1d2c49;
  border-radius:12px; padding:16px 16px 18px 16px; }
.mm-rail__label { font-family:'Space Grotesk',sans-serif; font-size:0.6rem; font-weight:600;
  letter-spacing:0.16em; text-transform:uppercase; color:#6b7c97; margin:13px 0 4px 0; }
...
.mm-chip { font-family:'JetBrains Mono',monospace; font-size:0.66rem; font-weight:600;
  padding:3px 8px; border-radius:6px; border:1px solid #233452; background:#0e1830; }
.mm-stage { display:inline-block; font-family:'Space Grotesk',sans-serif; font-weight:600;
  font-size:0.82rem; letter-spacing:0.06em; padding:3px 11px; border-radius:20px; }
.mm-scorebar { height:7px; border-radius:5px; background:#16223c; overflow:hidden; margin-top:7px; }
.mm-scorefill { height:100%; border-radius:5px; background:linear-gradient(90deg,#3884ff,#10b981);
  transition:width .4s ease; }
.mm-hb { font-family:'JetBrains Mono',monospace; font-size:0.7rem; color:#6b7c97; margin-top:12px; }

.mm-ledger-head { font-family:'Space Grotesk',sans-serif; font-size:0.7rem; font-weight:600;
  letter-spacing:0.16em; text-transform:uppercase; color:#6b7c97; margin:22px 0 10px 0;
  padding-bottom:7px; border-bottom:1px solid #1b2740; }
[data-testid="stDataFrame"] { border:0; }
[data-testid="stButton"] button { border-radius:8px; font-family:'Space Grotesk',sans-serif;
  font-weight:600; min-height:2.6rem; letter-spacing:0.05em;
...
def _launch_engine(config, reset_state):
    try:
        if reset_state:
            ...
            if history and history[0].status == "OPEN":
                state.update_trade_outcome(
                    ...
                    "Engine restarted while this contract was active. Check the Deriv statement.")
        ...
        state.set_error(f"Engine failed to initialize: {exc}")
        state.set_status("Bot could not start.")
        return False
...
def watchdog_fragment():
    ...
    if count >= MAX_AUTO_RESTARTS:
        ...
        state.set_running(False)
        state.set_error(
            f"Engine stopped {MAX_AUTO_RESTARTS} times in a row. "
            "Auto-restart paused — press START to retry.")
        state.set_status("Auto-restart paused after repeated failures.")
        st.rerun()
        return
    ...
    state.set_status(
        f"Engine stopped unexpectedly — auto-restarting (attempt {count + 1}/{MAX_AUTO_RESTARTS})...")
...
with st.sidebar:
    ...
    st.caption("Max 10 trades/day. No barriers — direction only.")
...
def ledger_fragment():
    ...
    for t in history[:50]:
        ...
        records.append({"Time": t.timestamp, "Dir": t.direction, "Stake": t.stake,
                        "Entry": t.entry_price, "Status": t.status, "P&L": pnl_str,
...
def journal_fragment():
    journal = get_journal()
    st.markdown('<div class="mm-ledger-head">Strategy Journal · every 15m decision</div>', unsafe_allow_html=True)
    csv_bytes = journal.to_csv_bytes()
    if csv_bytes:
        st.download_button(
            "⬇ Download journal (CSV) — opens in Excel / Google Sheets",
            data=csv_bytes, file_name="momentummaster_journal.csv", mime="text/csv",
            width="stretch")
    rows = journal.read_rows()
    if not rows:
        st.caption("No evaluations yet. The journal records every 15m candle decision — taken or rejected — with the full score breakdown and reason.")
        return
    df = pd.DataFrame(rows).tail(40).iloc[::-1]
    cols = ["timestamp_utc", "direction", "trend", "taken", "score", "threshold",
            "rejection_reason", "outcome", "pnl"]
