...
stake plan, results, and daily cap).

The price panel is a themed 5-minute candlestick chart (with a tick-sparkline
fallback for the first ~30s).

The Decision Log records every trigger-candle review, with the market column,
plus the result and reason of every setup, and offers a CSV export.

This version fixes Streamlit blank-page compatibility issues by replacing
unsupported width="stretch" usage with use_container_width=True.
"""
import asyncio
import html
import os
...
def _glitch(where, exc):
    st.markdown(
        f'<div class="mm-glitch">'
        f'<div class="t">⚠ {where} hit a snag</div>'
        f'<div class="s">The bot is fine — this is a rendering edge case, not a trading '
        f'fault. Details are in the expander; the controls stay live.</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
def watchdog_fragment():
    ...
    if count >= MAX_AUTO_RESTARTS:
        ...
        state.set_running(False)
        state.set_error(
            f"The engine stopped {MAX_AUTO_RESTARTS} times in a row — "
            "auto-restart is paused. Press Start to try again."
        )
    ...
    state.set_status(
        f"The engine stopped unexpectedly — restarting "
        f"(attempt {count + 1}/{MAX_AUTO_RESTARTS})…"
    )
...
with st.sidebar:
    ...
    entry_tf = ENTRY_TIMEFRAME_BY_DURATION.get(
    ...
    st.caption(
        f"Steps: {' → '.join(f'{s:.2f}' for s in stakes)}  ·  "
        f"max exposure {sum(stakes):.2f}{ccy_tag}"
    )
    ...
    _sens_val = (
        DEFAULT_STRATEGY_SENSITIVITY
        if DEFAULT_STRATEGY_SENSITIVITY in _sens_opts
        else _sens_opts[0]
    )
...
<div class="mm-header">
    <div>
        <div class="mm-logo">Momentum <span class="mm-dot">·</span>Master
            <span style="color:#6b7c97;font-weight:500;font-size:0.8rem;letter-spacing:0.1em;">TF</span>
        </div>
...
def status_fragment():
    try:
        ...
        st.markdown(
            f'<div class="mm-strip {cls}">{dot}{text}</div>',
            unsafe_allow_html=True,
        )
...
def metrics_fragment():
    try:
        ...
        wr_accent = (
            ...
            if wr < 45 and stats["total_trades"] > 0
            else "#3884ff"
        )
        ...
        st.markdown(
            f'<div class="mm-kpi-grid">{cards}</div>',
            unsafe_allow_html=True,
        )
...
def chart_fragment():
    try:
        ...
        if candles:
            ...
            fig.add_trace(
                go.Scatter(
                    x=[times.iloc[-1] if hasattr(times, "iloc") else times[-1]],
                    y=[last],
                    mode="markers",
...
<div class="mm-rail">
    ...
    <span class="mm-stage" style="color:{sc};background:{sbg};border:1px solid {sc}55;">
        {html.escape(str(stage_label))}
    </span>

    <div class="mm-rail__label">Setup score</div>
    <div style="font-family:'JetBrains Mono',monospace;font-weight:700;color:#eef3fb;">
        {score} / {SCORE_MAX}
    </div>
    <div class="mm-scorebar">
        <div class="mm-scorefill" style="width:{pct}%;"></div>
    </div>
...
def ledger_fragment():
    try:
        st.markdown(
            '<div class="mm-ledger-head">Trades</div>',
            unsafe_allow_html=True,
        )
        ...
        if not history:
            st.markdown(
                ...
                "contract length and the trigger candle confirms it."
                "</div>",
                unsafe_allow_html=True,
            )
            return
        ...
        styled = (
            df.style.map(st_status, subset=["Result"])
...
def journal_fragment():
    try:
        ...
        if not rows:
            st.caption(
                "Every trigger-candle review is recorded here — whether it traded or "
                "stood aside — along with the result and the reason."
            )
            return
