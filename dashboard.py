...
intends it to run (resuming martingale / P&L / daily cap). The Strategy Journal
section shows the complete decision + execution log (every 15m look and every
taken signal's terminal outcome + reason) and offers a CSV download.
"""
import asyncio
import html
import os
...
.mm-sub{font-family:'JetBrains Mono',monospace;font-size:0.7rem;color:#6b7c97;letter-spacing:0.04em;margin-top:3px;}
...
[data-testid="stButton"] button{border-radius:8px;font-family:'Space Grotesk',sans-serif;font-weight:600;min-height:2.6rem;letter-spacing:0.05em;transition:filter .15s ease,transform .12s ease;}
...
def journal_fragment():
    journal = get_journal()
    st.markdown('<div class="mm-ledger-head">Strategy Journal · every 15m decision + outcome</div>', unsafe_allow_html=True)
    ...
    if not rows:
        st.caption("No evaluations yet. The journal records every 15m candle decision — taken or rejected — plus the outcome and reason of every taken signal.")
        return
