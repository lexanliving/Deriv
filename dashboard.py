"""
dashboard.py
Streamlit dashboard for MomentumMaster.
The trading engine runs in a background asyncio thread; the UI is secondary.
"""
...
[data-testid="stMetricValue"] { font-size: 2.2rem !important; font-weight: 800; letter-spacing: -0.02em; }
[data-testid="stMetricLabel"] {
    font-size: 0.72rem !important; color: #6e7681; font-weight: 600;
...
with st.sidebar:
    ...
    st.caption(
        ...
        f"MTF {preset['mtf_min_agreement']}/3 | Score {preset['entry_score_threshold']}/14"
    )

    st.divider()
    st.markdown(
        "<div style='font-size:0.72rem;color:#6e7681;font-weight:600;letter-spacing:0.1em;"
        "text-transform:uppercase;margin-bottom:8px;'>Barriers</div>",
        unsafe_allow_html=True,
    )
...
with col_state_panel:
    ...
    if tf_biases:
        ...
        if tf_parts:
            st.markdown('<div style="margin-top:6px;display:flex;gap:10px;">' + " | ".join(tf_parts) + "</div>", unsafe_allow_html=True)
    ...
    if last_tick_time is not None:
        ...
        st.markdown(
            '<div style="margin-top:4px;font-size:0.72rem;color:#ff4d4f;">&#9679; no ticks received yet</div>',
            unsafe_allow_html=True,
        )
    ...
    if cooldown_remaining > 0:
        st.markdown(
            f'<div style="color:#f0b429;font-size:0.75rem;font-weight:600;margin-top:6px;">Cooldown: {int(cooldown_remaining)}s</div>',
            unsafe_allow_html=True,
        )
    elif consecutive_losses > 0:
        st.markdown(
            f'<div style="color:#58a6ff;font-size:0.75rem;font-weight:600;margin-top:6px;">Consecutive losses: {consecutive_losses} | Ready</div>',
            unsafe_allow_html=True,
        )
