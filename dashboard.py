...
with st.sidebar:
    ...
    if DYNAMIC_BARRIER_ENABLED:
        st.caption(
            "Dynamic barrier sizing is ON (config.py): the barrier below is used "
            "only as a fallback when there isn't yet enough tick history to size "
            "one from recent volatility. Actual per-trade barriers will usually "
            f"differ (bounded between {DYNAMIC_BARRIER_FLOOR} and {DYNAMIC_BARRIER_CEIL})."
        )

    barrier_buy_input = st.text_input(
        "Buy Barrier Offset (fallback)",
        value=BARRIER_BUY,
        help="Positive offset for Touch-Up trades (e.g., +0.08). Used as-is only when dynamic barrier sizing is off or tick history is insufficient.",
    )

    barrier_sell_input = st.text_input(
        "Sell Barrier Offset (fallback)",
        value=BARRIER_SELL,
        help="Negative offset for Touch-Down trades (e.g., -0.08). Used as-is only when dynamic barrier sizing is off or tick history is insufficient.",
    )
