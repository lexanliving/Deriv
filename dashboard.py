"""
dashboard.py
MomentumMaster — fragment-driven Streamlit trading terminal.

Live regions refresh themselves via st.fragment (status/metrics 2s, chart 5s,
ledger 10s). The full script only re-runs on user interaction — never on a
timer — so the UI stays out of the trading engine's way 24/7.
"""
...
def start_bot(
    ...
    real_execution_confirmed, initial_stake, max_steps, barrier_buy, barrier_sell,
    strategy_sensitivity, martingale_multiplier, symbol, symbol_display,
):
    ...
    engine = TradingEngine(
        ...
        barrier_buy=barrier_buy, barrier_sell=barrier_sell,
...
with st.sidebar:
    ...
    market_display = st.selectbox(
        "Market", options=market_options, index=market_index, disabled=state.is_running
    )
    selected_symbol = AVAILABLE_MARKETS[market_display]
    st.caption("Synthetic 1s indices are available on every Deriv account.")
    ...
    st.divider()
    st.markdown("<div style='font-size:0.72rem;color:#7c8ba1;font-weight:600;letter-spacing:0.12em;"
                "text-transform:uppercase;margin-bottom:8px;'>Barriers</div>", unsafe_allow_html=True)

    barrier_buy_input = st.text_input("Buy Barrier", value=BARRIER_BUY, disabled=state.is_running)
    barrier_sell_input = st.text_input("Sell Barrier", value=BARRIER_SELL, disabled=state.is_running)
    st.caption("Match the barrier to the index's price scale.")
    ...
    if start_pressed:
        if not selected_account:
            ...
            start_bot(
                ...
                barrier_buy=barrier_buy_input, barrier_sell=barrier_sell_input,
                strategy_sensitivity=strategy_sensitivity,
                martingale_multiplier=martingale_multiplier,
                symbol=selected_symbol, symbol_display=market_display,
            )
            st.rerun()
    ...
    st.caption(
        ...
        f"**Contract:** Touch | {CONTRACT_DURATION} ticks"
    )
...
<div class="terminal-header">
    <div>
        <div class="terminal-logo">MomentumMaster</div>
        <div class="terminal-subtitle">Tick Momentum | {html.escape(market_display)} ({html.escape(selected_symbol)})</div>
    </div>
...
def state_panel_fragment():
    ...
    if current_price:
        st.markdown(f'<div class="state-value" style="font-size:1.35rem;">{current_price:.4f}</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        f'<div class="state-value {mtf_cls}">{mtf} <span style="font-size:0.72rem;color:#7c8ba1;">({mtf_agreement}/3)</span></div>',
        unsafe_allow_html=True,
    )
    ...
    if micro:
        m_color = "#34d399" if micro == "UP" else "#fb7185"
        m_icon = "▲" if micro == "UP" else "▼"
        st.markdown(
            f'<div style="margin-top:4px;"><span style="color:#7c8ba1;font-size:0.7rem;">1m flow</span>: '
    ...
    if last_tick_time is not None:
        ...
        st.markdown(
            ...
            f'<span style="color:{hb_color};">&#9679;</span> {total_ticks:,} ticks processed &middot; {hb_label}</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown('<div style="margin-top:10px;font-size:0.72rem;color:#fb7185;">&#9679; no ticks received yet</div>', unsafe_allow_html=True)
