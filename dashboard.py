...
intends it to run — resuming without resetting martingale, P&L, or the daily
trade cap. On shutdown the thread drains its tasks before closing the loop.
"""
import asyncio
import html
import os
...
def _today_filled_trade_count(state_obj) -> int:
    """Count today's trades that actually reached a buy (have a contract_id),
    so the daily cap survives a restart."""
...
def _launch_engine(config, reset_state):
    """reset_state=True -> fresh START; False -> watchdog resume (keeps state)."""
    try:
        if reset_state:
...
def stop_bot():
    st.session_state.should_run = False  # tell the watchdog NOT to revive it
    state.request_stop()
