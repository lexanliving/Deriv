...
with st.sidebar:
    ...
    if SESSION_MAX_DRAWDOWN is not None:
        st.info(
            f"Session drawdown limit: **{account_currency} {SESSION_MAX_DRAWDOWN:.2f}** — "
            f"the bot will stop entering new trades once cumulative P&L falls to this level."
        )
    else:
        st.warning(
            "Session drawdown limit is **disabled**. Running without a circuit breaker is not recommended."
        )
