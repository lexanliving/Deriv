...
stake plan, results, and daily cap). The price panel is a themed 5-minute
candlestick chart (with a tick-sparkline fallback for the first ~30s). The
Decision Log records every 15-minute review (with the market column) plus the
result and reason of every setup, and offers a CSV export. Every widget is
value-safe and every live fragment is crash-wrapped so a glitch never blanks
the page.
"""
import asyncio
import html
...
with st.sidebar:
    ...
    st.caption(f"Needs {preset.get('entry_score_threshold', SCORE_MAX)}/{SCORE_MAX} · 15m ADX ≥ {preset.get('entry_adx_floor', 15)} · {regime_note}")
...
# ---------------------------------------------------------------------------
# Header (crash-wrapped; safe fall-back defaults so a sidebar short-circuit
# can never leave a variable undefined)
# ---------------------------------------------------------------------------
try:
    ...
    try:
        ...
        if candles:
            ...
            st.markdown(
                ...
                unsafe_allow_html=True)
            fig = go.Figure()
            # faint close-price glow line behind the candles for depth
            fig.add_trace(go.Scatter(
                x=times, y=cl, mode="lines",
            ...
            # pulsing last-price marker + dashed level
            fig.add_trace(go.Scatter(
            ...
            fig.update_layout(
                ...
                margin=dict(l=8, r=8, t=8, b=8), height=360, showlegend=False, dragmode="pan")
