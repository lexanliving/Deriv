...
A hedge-fund-grade trading terminal with:
  ...
  - Manual trade test button for code verification
...
# ---------------------------------------------------------------------------
def execute_manual_trade(signal: str):
    """
    Execute a manual test trade using the engine's _execute_trade path.
    Reuses the same safety guards, proposal fetching, buy submission,
    and contract monitoring as automated signals.
    """
    engine = st.session_state.get("engine_instance")
    if engine is None:
        st.error("No active engine. Start the bot first to connect to Deriv.")
        return

    if engine._trade_in_progress:
        st.warning("A trade is already in progress. Please wait for settlement.")
        return

    if not state.is_running:
        st.error("The bot must be running to place trades. Start the bot first.")
        return

    current_price = state.current_price
    if current_price == 0:
        st.error("No price data available yet. Wait for the tick stream.")
        return

    # Create a new event loop for the manual trade
    loop = asyncio.new_event_loop()
    try:
        loop.run_until_complete(engine._execute_trade(signal, current_price))
    finally:
        loop.close()

    st.success(f"Manual {signal} trade initiated at {current_price:.4f}")
...
with st.sidebar:
    ...
    if SESSION_MAX_DRAWDOWN is not None:
        st.caption(f"Drawdown limit: {account_currency} {SESSION_MAX_DRAWDOWN:.2f}")
    else:
        st.warning("No drawdown limit set.")
    ...
    # ---- Manual Trade Test Button ----
    st.markdown(
        "<div style='font-size:0.72rem;color:#6e7681;font-weight:600;letter-spacing:0.1em;"
        "text-transform:uppercase;margin-bottom:8px;'>Manual Trade Test</div>",
        unsafe_allow_html=True,
    )

    st.caption("Send a BUY or SELL order manually to verify trade placement.")

    col_buy, col_sell = st.columns(2)
    with col_buy:
        manual_buy = st.button("BUY", type="primary", width='stretch')
    with col_sell:
        manual_sell = st.button("SELL", type="secondary", width='stretch')

    if manual_buy:
        execute_manual_trade("BUY")
        st.rerun()

    if manual_sell:
        execute_manual_trade("SELL")
        st.rerun()
...
with col_tc:
    ...
    if stats["total_trades"] > 0:
        st.caption(f"{stats['wins']}W / {stats['losses']}L | Current stake: {stats['current_stake']:.2f} {display_currency}")
...
with col_state_panel:
    ...
    stage_colors = {"IDLE": "#6e7681", "PULLBACK": "#f0b429", "MOMENTUM": "#58a6ff", "SIGNAL": "#00d4aa"}
    st.markdown(
    ...
    st.markdown(
        ...
        f'<div style="color:#e6edf3;font-weight:700;font-size:1rem;">{score}/14</div>',
        unsafe_allow_html=True,
    )
    ...
    if cooldown:
        st.markdown(
            '<div style="color:#f0b429;font-size:0.75rem;font-weight:600;margin-top:6px;">Cooldown Active</div>',
            unsafe_allow_html=True,
        )
...
if trade_history:
    ...
    st.markdown(
        ...
        'No trades yet. Start the bot and wait for a qualifying signal, or use the Manual Trade buttons to test.</div>',
        unsafe_allow_html=True,
    )
