...
with col_chart:
    st.markdown("### Performance & Speed")
    st.info("📊 Chart disabled for maximum execution performance.")
    
    current_price = state.current_price
    if current_price:
        st.metric("Live Market Price", f"{current_price:.4f}")
        
    # Show a small text-based trend indicator
    ticks = state.get_recent_ticks()
    if len(ticks) >= 5:
        last_5 = ticks[-5:]
        diff = last_5[-1] - last_5[0]
        trend_arrow = "▲" if diff > 0 else "▼"
        st.markdown(f"**Short-term trend (5-tick):** {trend_arrow} ({diff:+.4f})")
...
# ---------------------------------------------------------------------------
if state.is_running:
    # Refresh every 33 ticks (approx 33 seconds) to reduce UI overhead
