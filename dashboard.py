...
# Support both layouts: dashboard.py + src/*.py, or all files in one folder.
...
def _run_engine_in_thread(engine: TradingEngine, loop: asyncio.AbstractEventLoop, state: StateManager):
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(engine.run())
    except Exception as exc:
        try:
            state.set_error(f"Trading engine crashed: {exc}")
            state.set_status("Bot stopped because the engine failed. Check logs.")
        finally:
...
def start_bot(
    ...
    state.clear_error()
    state.set_status("Starting engine...")
    ...
    thread = threading.Thread(
        target=_run_engine_in_thread, args=(engine, loop, state),
        daemon=True, name="TradingEngineThread",
    )
    try:
...
def stop_bot():
    ...
    if thread is None or not thread.is_alive():
        state.set_running(False)
        state.set_status("Bot stopped.")


@st.fragment(run_every=2.0)
def watchdog_fragment():
    thread = st.session_state.get("engine_thread")
    if state.is_running and (thread is None or not thread.is_alive()):
        state.set_error("Engine thread exited unexpectedly. Bot stopped.")
        state.set_status("Bot stopped.")
        state.set_running(False)
        st.rerun()
